-- Complete CodeMap Database Schema
-- This migration includes all necessary tables, functions, policies, and configurations
...
-- Create project submission status enum (if not exists)
DO $$ BEGIN
...
-- Create classroom_students junction table (if not exists)
...
-- Create concept_maps table (if not exists)
...
-- Create project_submissions table (if not exists)
...
CREATE INDEX IF NOT EXISTS idx_concept_maps_owner_id ON concept_maps(owner_id);
CREATE INDEX IF NOT EXISTS idx_concept_maps_shared_with_classroom_id ON concept_maps(shared_with_classroom_id);
CREATE INDEX IF NOT EXISTS idx_project_submissions_student_id ON project_submissions(student_id);
CREATE INDEX IF NOT EXISTS idx_project_submissions_classroom_id ON project_submissions(classroom_id);
CREATE INDEX IF NOT EXISTS idx_project_submissions_analysis_status ON project_submissions(analysis_status);
...
RETURNS TEXT AS $$
BEGIN
    RETURN 'CM' || substr(md5(random()::text), 1, 8);
END;
$$ LANGUAGE plpgsql;
...
DROP TRIGGER IF EXISTS update_concept_maps_updated_at ON concept_maps;
