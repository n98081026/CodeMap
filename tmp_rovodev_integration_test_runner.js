...
try {
  ...
  const testResults = {
    ...
    conceptMap: runTestSuite(
      'Concept Map Flow',
      'src/tests/integration/concept-map-flow.test.ts'
    ),
    projectAnalysis: runTestSuite(
      'Project Analysis Flow',
      'src/tests/integration/project-analysis-flow.test.ts'
    ),
    classroom: runTestSuite(
      'Classroom Management Flow',
      'src/tests/integration/classroom-management-flow.test.ts'
    ),
  };
  ...
  Object.entries(testResults).forEach(([suite, passed]) => {
    const status = passed ? '✅' : '❌';
    console.log(
      `${status} ${suite.charAt(0).toUpperCase() + suite.slice(1)} Flow`
    );
  });
