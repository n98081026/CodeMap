...
export const graphologyCommunityDetectionTool = defineTool(
  {
    ...
    description: 'Detects communities (clusters) of nodes in a concept map using the Louvain algorithm. Returns communities with 2 to 5 nodes.',
  ...
  async (mapData) => {
    try {
      ...
      if (graphInstance.order < 2) { // Graph order is number of nodes
         console.warn(`[CommunityDetector] Graph has ${graphInstance.order} nodes. Louvain typically requires more structure.`);
         return { detectedCommunities: [] };
      }
      ...
      try {
        ...
        console.warn(`[CommunityDetector] Louvain algorithm error: ${louvainError.message}. This can happen with disconnected graphs or very small components.`);
        // Return empty communities or a specific error if Louvain fails internally
        return { detectedCommunities: [], error: `Louvain algorithm execution failed: ${louvainError.message}` };
      }
      ...
      graphInstance.forEachNode((nodeId, attributes) => {
        ...
        if (communityId !== undefined) { // Check if community attribute was assigned
      ...
      if (!communityDataFound && graphInstance.order > 0) {
         console.warn("[CommunityDetector] Louvain ran, but no nodes were assigned a community attribute.");
         return { detectedCommunities: [] };
      }
      ...
      console.error('[CommunityDetector] Error in graphologyCommunityDetectionTool:', e);
      return {
        detectedCommunities: [],
