...
describe('projectStructureAnalyzerTool', () => {
  ...
  const createMockFileFetcherOutput = (
    fileName: string,
    fileContent: string,
    contentType: string = 'text/x-python',
...
`;
      ...
      expect(result.analysisSummary).toContain(
        "Python file 'test_script.py' (AST analysis)"
      );
      ...
      expect(funcNode?.details).toContain(
        'Mocked summary for function top_level_function'
      );
      expect(funcNode?.details).toContain('Params: param1: int, param2: str');
      expect(funcNode?.details).toContain('Decorators: my_decorator');
      expect(funcNode?.structuredInfo.comments).toBe(
        'This is a docstring for a top-level function.'
      );
      ...
      expect(initMethodNode?.details).toContain(
        'Mocked summary for function __init__'
      );
      ...
      expect(methodOneNode?.details).toContain(
        'Mocked summary for function method_one'
      );
      expect(methodOneNode?.details).toContain(
        'Params: self, multiplier: float'
      );
      expect(methodOneNode?.structuredInfo.comments).toBe(
        'Multiplies instance_var.'
      );
      expect(methodOneNode?.lineNumbers).toBe('20-24');
      expect(methodOneNode?.details).toContain(
        'Local Calls: self.another_method() (line 22)'
      );
      ...
      expect(anotherMethodNode?.details).toContain(
        'Mocked summary for function another_method'
      );
      ...
      expect(anotherFuncNode?.details).toContain(
        'Local Calls: top_level_function() (line 30)'
      );
...
from ..parent import parent_module
    ...
    it('should handle an empty Python file', async () => {
      ...
      expect(result.analysisSummary).toContain(
        "Python file 'empty.py' (AST analysis): Found 0 top-level functions, 0 classes, and 0 import statements. Detected 0 local calls."
      );
...
class MyBrokenClass
  pass
      ...
      expect(result.analysisSummary).toContain(
        'Regex Analysis: Functions: 1, Classes: 1, Imports: 0.'
      ); // Based on regex
  ...
  it('should handle errors from supabaseFileFetcherTool gracefully', async () => {
    ...
    expect(result.analysisSummary).toContain(
      'Error fetching file: Supabase fetch failed'
    );
