...
export const SharedNeighborsEdgeInputSchema = MapDataSchema.extend({
  jaccardThreshold: z.number().min(0).max(1).optional().default(0.3)
    .describe("Minimum Jaccard Index for suggesting an edge (0.0 to 1.0)."),
  maxCandidates: z.number().int().positive().optional().default(5)
    .describe("Maximum number of candidate edges to return, sorted by Jaccard Index."),
});
export type SharedNeighborsEdgeInput = z.infer<typeof SharedNeighborsEdgeInputSchema>;
...
export const graphologySharedNeighborsEdgeTool = defineTool(
  {
    ...
    description: 'Detects potential edges between nodes based on shared neighbors using Jaccard Index.',
  ...
  async (input: SharedNeighborsEdgeInput) => {
    try {
      ...
      if (graphInstance.order < 2) { // graphInstance.order is the number of nodes
        return { candidateEdges: [] };
      }
      ...
      for (let i = 0; i < allNodeIds.length; i++) {
        for (let j = i + 1; j < allNodeIds.length; j++) {
          ...
          if (graphInstance.hasEdge(nodeA_id, nodeB_id) || graphInstance.hasEdge(nodeB_id, nodeA_id)) {
            continue;
          }
          ...
          const intersection = new Set([...neighborsA].filter(id => neighborsB.has(id)));
          ...
          const jaccardIndex = unionSize > 0 ? intersection.size / unionSize : 0;
