...
import { summarizeCodeElementPurposeFlow, type SummarizeCodeElementInput } from '@/ai/flows';
// Assuming these types are exported from project-analyzer-tool.ts or a shared types file
import type { DetailedNode, ExtractedCodeElement } from './project-analyzer-tool';
...
export interface SummarizationTaskInfo {
  ...
  originalNodeInfo: Omit<ExtractedCodeElement, 'astNode' | 'localCalls' | 'semanticPurpose'>;
...
export async function batchSummarizeElements(
  ...
  const summarizationPromises = tasks.map(task =>
    summarizeCodeElementPurposeFlow.run(task.inputForFlow)
      ...
      .catch(error => {
        console.warn(`[AST Utils] Error summarizing ${task.nodeType} ${task.originalNodeInfo.name} in ${task.inputForFlow.filePath || fileName}: ${error.message}`);
        return {
          uniqueId: task.uniqueId,
...
export function createDetailedNodeFromExtractedElement(
  ...
  if (element.kind === 'function' || element.kind === 'method') {
    ...
    if(element.implementedInterfaces && element.implementedInterfaces.length > 0) details += `Implements: ${element.implementedInterfaces.join(', ')}. `;
    ...
    if (element.classMethods && element.classMethods.length > 0) {
  ...
  } else if (element.kind === 'variable' || element.kind === 'property') { // Also covers module-level variables
      ...
      if (element.isExported !== undefined) details += `Exported: ${element.isExported}. `;
  ...
  if (element.comments) details += `Docstring/Comments: Present (see structuredInfo).`;
  ...
  return {
    ...
    lineNumbers: (element.startLine && element.endLine) ? `${element.startLine}-${element.endLine}` : undefined,
