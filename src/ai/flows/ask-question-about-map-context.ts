...
const MapContextNodeSchema = z.object({
  ...
  type: z.string().describe("The category or type of the node (e.g., 'class', 'function', 'concept')."),
  details: z.string().optional().describe("Additional descriptive text for the node."),
});
...
const MapContextEdgeSchema = z.object({
  ...
  label: z.string().optional().describe("Label describing the relationship of this edge."),
});

// --- Input Schema ---
export const AskQuestionAboutMapContextInputSchema = z.object({
  nodes: z.array(MapContextNodeSchema).describe("An array of key attributes for all nodes in the map."),
  edges: z.array(MapContextEdgeSchema).describe("An array of key attributes for all edges in the map."),
  userQuestion: z.string().describe("The user's question about the overall map context."),
  mapName: z.string().optional().describe("The name of the concept map, for context."),
});
export type AskQuestionAboutMapContextInput = z.infer<typeof AskQuestionAboutMapContextInputSchema>;

// --- Output Schema ---
export const AskQuestionAboutMapContextOutputSchema = z.object({
  answer: z.string().describe("The AI-generated answer to the user's question about the map."),
  error: z.string().optional().describe("Error message if the question could not be answered."),
});
export type AskQuestionAboutMapContextOutput = z.infer<typeof AskQuestionAboutMapContextOutputSchema>;
...
export const askQuestionAboutMapContextFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!input.nodes || input.nodes.length === 0) {
      return {
        answer: "The concept map is empty, so I cannot answer questions about it.",
    ...
    try {
      ...
      if (!output) {
        return {
          answer: "The AI was unable to generate an answer for your question about this map.",
      ...
      return {
        answer: "An error occurred while the AI was processing your question about the map.",
