...
export const suggestMapImprovementsFlow = defineFlow(
  ...
  async (mapData) => {
    console.log(
      `[suggestMapImprovementsFlow] Received map with ${mapData.nodes.length} nodes and ${mapData.edges.length} edges.`
    );
    if (mapData.nodes.length < 2) {
      console.log(
        '[suggestMapImprovementsFlow] Not enough nodes for meaningful suggestions.'
      );
    ...
    try {
      ...
      if (graphInstance.order > 0) {
        ...
        const centralities =
          graphAdapter.getBetweennessCentrality(graphInstance);
        ...
        if (Object.keys(communities).length > 0) {
          ...
          for (const [nodeId, communityId] of Object.entries(communities)) {
            ...
            communitySummary[communityKey].push(
              mapData.nodes.find((n) => n.id === nodeId)?.text || nodeId
            );
          }
        }
      ...
      console.error(
        '[suggestMapImprovementsFlow] Error during graph analysis:',
        e.message
      );
    ...
    const mapDataString = `Nodes:\n${JSON.stringify(
      mapData.nodes.map((n) => ({
        id: n.id,
        text: n.text,
        details: n.details?.substring(0, 100),
      })),
      null,
      2
    )}\n\nEdges:\n${JSON.stringify(
...
${JSON.stringify(
  mapData.edges.map((e) => ({ from: e.source, to: e.target, label: e.label })),
  null,
  2
)}
...
`;
    ...
    try {
      ...
      console.log(
        `[suggestMapImprovementsFlow] LLM generated ${rawEdges.length} raw edge suggestions and ${rawGroups.length} raw group suggestions.`
      );

      const existingNodeIds = new Set(mapData.nodes.map((n) => n.id));
      const existingEdgesSet = new Set(
        mapData.edges.map((e) => `${e.source}-${e.target}`)
      );
      const existingEdgesReverseSet = new Set(
        mapData.edges.map((e) => `${e.target}-${e.source}`)
      );
      ...
      return {
        suggestedEdges: validEdgeSuggestions,
        suggestedGroups: validGroupSuggestions,
      };
    } catch (error) {
      console.error(
        '[suggestMapImprovementsFlow] Error during LLM call or processing:',
        error
      );
      return { suggestedEdges: [], suggestedGroups: [] };
    }
