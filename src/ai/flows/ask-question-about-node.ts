...
export const AskQuestionAboutNodeInputSchema = z.object({
  ...
  nodeDetails: z
    .string()
    .optional()
    .describe('The detailed description or content of the node.'),
  nodeType: z
    .string()
    .optional()
    .describe(
      "The type of the node (e.g., 'js_function', 'py_class', 'ai-summary-node')."
    ),
  userQuestion: z
    .string()
    .min(5)
    .describe("The user's question about this specific node."),
});
export type AskQuestionAboutNodeInput = z.infer<
  typeof AskQuestionAboutNodeInputSchema
>;
...
export const AskQuestionAboutNodeOutputSchema = z.object({
  answer: z
    .string()
    .describe(
      "The AI's answer to the user's question, explained in simple terms."
    ),
  error: z
    .string()
    .optional()
...
export type AskQuestionAboutNodeOutput = z.infer<
  typeof AskQuestionAboutNodeOutputSchema
>;

export async function askQuestionAboutNode(
  input: AskQuestionAboutNodeInput
  ...
  async (input) => {
    try {
      if (!input.userQuestion.trim() || input.userQuestion.length < 5) {
        return {
          answer:
            'Your question seems a bit short. Could you please provide more details?',
          error: 'Question too short or empty.',
        };
      }
      ...
      if (!output.answer && output.error) {
        ...
        output.answer =
          "I'm unable to provide an answer for that question based on the current node's information.";
      ...
      return {
        answer:
          'An unexpected error occurred while trying to answer your question.',
