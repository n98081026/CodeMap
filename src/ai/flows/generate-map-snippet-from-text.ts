...
const GenerateMapSnippetInputSchema = z.object({
  text: z
    ...
    .min(50, {
      message:
        'Text should be at least 50 characters to generate a meaningful snippet.',
    })
    .describe(
      'A block of text (e.g., meeting notes, documentation paragraph) to analyze and structure into a concept map snippet.'
    ),
});
export type GenerateMapSnippetInput = z.infer<
  typeof GenerateMapSnippetInputSchema
>;
...
const NodeSchema = z.object({
  ...
  type: z
    ...
    .optional()
    .describe(
      'An optional type for the node (e.g., "key_idea", "person", "action_item", "system_component"). Default if not provided could be "text-derived-concept".'
    ),
  details: z
    .string()
    .optional()
    .describe(
      'Optional brief details or elaboration for the node, extracted or inferred from the text.'
    ),
});
...
const EdgeSchema = z.object({
  sourceText: z
    .string()
    .describe(
      'The exact text label of one of the generated source nodes for this edge.'
    ),
  targetText: z
    .string()
    .describe(
      'The exact text label of one of the generated target nodes for this edge.'
    ),
  relationLabel: z
    .string()
    .describe(
      'A concise label describing the relationship (e.g., "influences", "discusses", "responsible_for", "part_of").'
    ),
});
...
const GenerateMapSnippetOutputSchema = z.object({
  nodes: z
    ...
    .max(7)
    .describe(
      'An array of 2 to 7 generated concept nodes representing key entities or ideas from the text.'
    ),
  edges: z
    .array(EdgeSchema)
    .min(1)
    ...
    .optional()
    .describe(
      'An optional array of 1 to 5 suggested edges connecting the generated nodes, representing relationships found in the text.'
    ),
});
export type GenerateMapSnippetOutput = z.infer<
  typeof GenerateMapSnippetOutputSchema
>;

export async function generateMapSnippetFromText(
  input: GenerateMapSnippetInput
): Promise<GenerateMapSnippetOutput> {
