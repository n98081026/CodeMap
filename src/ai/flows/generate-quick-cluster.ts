...
const GenerateQuickClusterInputSchema = z.object({
  prompt: z.string().describe('The user prompt or topic to generate a concept cluster around.'),
});
export type GenerateQuickClusterInput = z.infer<typeof GenerateQuickClusterInputSchema>;

const NodeSchema = z.object({
  text: z.string().describe('The concise label for the concept node.'),
  type: z.string().optional().describe('An optional type for the node (e.g., "idea", "question", "task"). Default if not provided could be "ai-generated".'),
  details: z.string().optional().describe('Optional brief details or elaboration for the node.'),
});

const EdgeSchema = z.object({
  sourceText: z.string().describe('The text label of the source node for this edge. Must match one of the generated node texts.'),
  targetText: z.string().describe('The text label of the target node for this edge. Must match one of the generated node texts.'),
  relationLabel: z.string().describe('A concise label describing the relationship (e.g., "leads to", "supports", "is part of").'),
});
...
export async function generateQuickCluster(input: GenerateQuickClusterInput): Promise<GenerateQuickClusterOutput> {
