...
import { z } from 'zod';
import { suggestMapImprovementFlow, MapImprovementSuggestionSchema } from './suggest-map-improvement';
import { suggestNodeGroupCandidatesFlow, NodeGroupSuggestionSchema } from './suggest-node-group-candidates';
...
import { MapDataSchema, NewIntermediateNodeDataSchema as MapImprovementNewIntermediateNodeDataSchema } from './suggest-map-improvement'; // Import specific schema for parsing
import {
...
export const fetchAllStructuralSuggestionsFlow = defineFlow(
  ...
  async (mapData) => {
    ...
    try {
      const improvementSuggestion = await suggestMapImprovementFlow.run(mapData);
      if (improvementSuggestion) {
        ...
        if (improvementSuggestion.type === 'ADD_EDGE') {
          ...
          console.warn(`Unknown suggestion type from suggestMapImprovementFlow: ${improvementSuggestion.type}`);
          return suggestionsList; // Or handle error appropriately
        }

        suggestionsList.push({
          id: uuidv4(),
          type: improvementSuggestion.type as 'ADD_EDGE' | 'NEW_INTERMEDIATE_NODE' | 'FORM_GROUP',
          data: validatedData,
    ...
    try {
      ...
      if (groupCandidate && groupCandidate.nodeIdsToGroup && groupCandidate.suggestedParentName) {
        const isDuplicateGroupSuggestion = suggestionsList.some(s =>
          s.type === 'FORM_GROUP' &&
          s.data &&
          Array.isArray((s.data as any).nodeIdsToGroup) && // Added type assertion for data
          JSON.stringify((s.data as any).nodeIdsToGroup.slice().sort()) === JSON.stringify(groupCandidate.nodeIdsToGroup.slice().sort())
        );
    ...
    try {
      const enhancedEdgeResult = await suggestGraphologyEnhancedEdgeFlow.run(mapData);
      if (enhancedEdgeResult && enhancedEdgeResult.type === 'ADD_EDGE') {
        const isDuplicateEdge = suggestionsList.some(s =>
          s.type === 'ADD_EDGE' &&
          (s.data as any).sourceNodeId === enhancedEdgeResult.data.sourceNodeId &&
          (s.data as any).targetNodeId === enhancedEdgeResult.data.targetNodeId
        );
        if (!isDuplicateEdge) {
           const validatedEdgeData = AddEdgeDataSchema.parse(enhancedEdgeResult.data);
           suggestionsList.push({
            id: uuidv4(),
            type: 'ADD_EDGE',
    ...
    try {
      const intermediateNodeSuggestion = await suggestGraphologyIntermediateNodeFlow.run(mapData);
      if (intermediateNodeSuggestion && intermediateNodeSuggestion.type === 'NEW_INTERMEDIATE_NODE') {
        // Parse with the schema from suggest-map-improvement which includes originalEdgeId
        const parsedData = MapImprovementNewIntermediateNodeDataSchema.parse(intermediateNodeSuggestion.data);

        let isDuplicateIntermediate = false;
        if (parsedData.originalEdgeId) {
            isDuplicateIntermediate = suggestionsList.some(s =>
        ...
        console.error("Error running suggestGraphologyIntermediateNodeFlow:", error);
    }
