...
test.describe('Suggest Relations AI Tool Flow', () => {
  ...
  const EXPECTED_RELATION_LABEL_PARTIAL_REGEX =
    /is a|includes|example of|type of|uses/i;
  ...
  test('should suggest relations and allow adding them to the map', async ({
    page,
  }) => {
    ...
    const relationItem = await editorPage.getSuggestedRelationItemByParts(
      NODE_TEXT_1,
      NODE_TEXT_2
    );
    await expect(relationItem).toBeVisible({ timeout: 20000 });
    console.log(
      `POM: Expected relation item containing "${NODE_TEXT_1}" and "${NODE_TEXT_2}" is visible.`
    );

    await expect(relationItem).toHaveText(
      EXPECTED_RELATION_LABEL_PARTIAL_REGEX
    );
