...
test.describe('Project Overview Mode Flow', () => {
  ...
  test('should activate, display, interact with, and deactivate project overview mode', async ({
    page,
  }) => {
    const overviewContainer = page.locator(
      "div[data-tutorial-id='project-overview-display-container']"
    ); // Keep this specific locator

    // 1. Activate Project Overview Mode using POM
    await expect(editorPage.toggleOverviewButton).toBeVisible();
    await expect(
      editorPage.toggleOverviewButton.locator('svg.lucide-eye')
    ).toBeVisible();
    await editorPage.toggleOverviewMode();

    // 2. Wait and verify the overview appears
    await expect(
      editorPage.toggleOverviewButton.locator('svg.lucide-eye-off')
    ...
    await expect(
      overviewContainer.locator('h2:has-text("Key Modules / Components")')
    ).toBeVisible();
    const firstModuleCard = overviewContainer.locator(
      '.grid > div:first-child'
    );
    await expect(firstModuleCard).toBeVisible({ timeout: 10000 });
    const moduleCardCount = await overviewContainer
      .locator('.grid > div')
      .count();
    expect(moduleCardCount).toBeGreaterThan(0);
    console.log(
      `POM: ${moduleCardCount} module cards are visible in the overview.`
    );
    ...
    console.log(
      'POM: Project Overview Display container is hidden after module click.'
    );
    await expect(editorPage.reactFlowPane).toBeVisible();
    console.log(
      'POM: React Flow pane is visible after exiting overview via module click.'
    );
    await expect(
      editorPage.toggleOverviewButton.locator('svg.lucide-eye')
    ).toBeVisible();
    ...
    await expect(overviewContainer).not.toBeVisible({ timeout: 10000 });
    console.log(
      'POM: Project Overview Display container is hidden after deactivation.'
    );
    await expect(editorPage.reactFlowPane).toBeVisible();
    ...
    console.log(
      "POM: Toggle overview button icon reverted to 'Eye' after deactivation."
    );
