...
test.describe('Extract Concepts AI Tool Flow', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    console.log(`POM: Node with text "${NODE_TEXT_TO_EXTRACT}" added for Extract Concepts test.`);
  });

  test('should extract concepts from a node and display them in the AI panel, then add to map', async ({ page }) => {
    ...
    const expectedConceptItem = await editorPage.getExtractedConceptItemByText(EXPECTED_CONCEPT_PARTIAL);
    await expect(expectedConceptItem).toBeVisible({ timeout: 15000 });
    console.log(`POM: Expected concept containing "${EXPECTED_CONCEPT_PARTIAL}" is visible in AI panel.`);

    // 5. Add the concept to the map and verify
    // The getExtractedConceptItemByText returns the item, now find checkbox within it
    const conceptCheckbox = expectedConceptItem.locator('button[role="checkbox"]');
    await expect(conceptCheckbox).toBeVisible({timeout: 5000});
    await conceptCheckbox.click();
    console.log("POM: Extracted concept checkbox (matching partial text) clicked.");
    ...
    const addedNodeTextContent = await expectedConceptItem.locator('span').first().textContent(); // Get text from the concept item itself
    ...
    const addedNodeOnCanvas = page.locator(`.react-flow__node:has-text("${cleanedAddedNodeText}")`);
    await expect(addedNodeOnCanvas).toBeVisible({timeout: 10000});
    console.log(`POM: Node with text "${cleanedAddedNodeText}" verified on canvas.`);
  });
});
