...
export async function navigateToCreateNewMap(page: Page) {
  ...
  await page
    .getByRole('link', { name: /concept maps|my maps|le mie mappe/i })
    .first()
    .click();
  await expect(page).toHaveURL(/.*\/student\/concept-maps/, { timeout: 10000 });
  console.log(
    `Navigated to student concept maps page. Current URL: ${page.url()}`
  );

  await page
    .getByRole('button', {
      name: /create new map|new map|crea nuova mappa|nuova mappa/i,
    })
    .click();
  console.log(`"Create New Map" button clicked. Navigating to editor...`);

  const editorToolbarAddNodeButton = page.locator(
 ...
 */
export async function addNodeToMap(
  page: Page,
  nodeText: string,
  nodeIndex: number = 0
) {
  ...
  const propertiesToggleButton = page.locator(
    '#tutorial-target-toggle-properties-button'
  );
  const nodeTextInput = page.locator(
  ...
  await expect(page.locator('.react-flow__node').nth(nodeIndex)).toBeVisible({
    timeout: 5000,
  });
  ...
  console.log(
    `Focus shifted from node text input for node at index ${nodeIndex}.`
  );
