...
export async function getSystemSettings(): Promise<SystemSettings> {
  const { data, error } = await supabase
    ...
    .select(
      'enable_ai_project_analysis, default_concept_map_visibility, max_project_file_size_mb'
    )
  ...
  if (data) {
    return {
      ...
      default_concept_map_visibility: data.default_concept_map_visibility as
        | 'public'
        | 'private',
  ...
  console.warn(
    'No system settings found in database, returning default values. Creating an initial settings row with defaults.'
  );
  ...
  try {
    ...
    if (insertedData) {
      return {
        ...
        default_concept_map_visibility:
          insertedData.default_concept_map_visibility as 'public' | 'private',
...
export async function updateSystemSettings(
  ...
  if (!data) {
    throw new Error(
      'Failed to update system settings: No data returned after upsert.'
    );
  }
