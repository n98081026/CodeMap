...
import { FolderKanban, PlusCircle, Loader2, AlertTriangle, ArrowLeft, ArrowRight } from "lucide-react";
...
export default function MySubmissionsPage() {
  ...
  const fetchSubmissions = useCallback(async (pageToFetch: number) => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      const response = await fetch(`/api/projects/submissions?studentId=${user.id}&page=${pageToFetch}&limit=${SUBMISSIONS_PER_PAGE}`);
      if (!response.ok) {
    ...
    if (error) {
      return (
        <EmptyState
          ...
          description={error}
          action={<Button onClick={() => fetchSubmissions(currentPage)}>Retry</Button>}
        />
      );
    }
