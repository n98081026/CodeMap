...
export async function DELETE(
  ...
  try {
        ...
        try {
          ...
          if (classroom.teacher_id !== user.id) {
            return NextResponse.json(
              {
                message:
                  'Forbidden: You are not the teacher of this classroom.',
              },
              { status: 403 }
            );
          }
          ...
          return NextResponse.json(
            { message: 'Error validating classroom ownership.' },
            { status: 500 }
          );
        }
      } else {
        // Other roles (e.g., Student) cannot remove students
        return NextResponse.json(
    ...
    console.error(
      `Remove Student from Classroom API error (Classroom ID: ${context.params.classroomId}, Student ID: ${context.params.studentId}):`,
      error
    );
    const errorMessage =
