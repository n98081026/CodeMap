...
export async function GET(request: Request, context: { params: { classroomId: string } }) {
  try {
    ...
    if (userRole !== UserRole.ADMIN && user.id !== classroom.teacher_id) {
      ...
      return NextResponse.json({ message: "Forbidden: Insufficient permissions to view this classroom." }, { status: 403 });
    }
...
export async function PUT(request: Request, context: { params: { classroomId: string } }) {
  try {
    ...
    const userRole = user.user_metadata?.role as UserRole;
    if (userRole !== UserRole.ADMIN && user.id !== classroomToUpdate.teacher_id) {
      return NextResponse.json({ message: "Forbidden: Insufficient permissions to update this classroom." }, { status: 403 });
    }
    ...
    if (!updatedClassroom) {
      ...
      return NextResponse.json({ message: "Classroom not found or update failed post-authorization" }, { status: 404 });
    }
...
export async function DELETE(request: Request, context: { params: { classroomId: string } }) {
  try {
    ...
    const userRole = user.user_metadata?.role as UserRole;
    if (userRole !== UserRole.ADMIN && user.id !== classroomToDelete.teacher_id) {
      return NextResponse.json({ message: "Forbidden: Insufficient permissions to delete this classroom." }, { status: 403 });
    }

    const deleted = await deleteClassroom(classroomId);
    if (!deleted) {
      // This case should ideally be covered by the previous check or service layer error
      return NextResponse.json({ message: "Classroom not found or deletion failed post-authorization" }, { status: 404 });
    }
