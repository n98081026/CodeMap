...
import { getConceptMapById, updateConceptMap, deleteConceptMap } from '@/services/conceptMaps/conceptMapService';
...
export async function GET(request: Request, context: { params: { mapId: string } }) {
  try {
    ...
    if (!map) {
      return NextResponse.json({ message: "Concept map not found" }, { status: 404 });
    }
...
export async function PUT(request: Request, context: { params: { mapId: string } }) {
  try {
    ...
    const updates = await request.json() as { name?: string; mapData?: ConceptMapData; isPublic?: boolean; sharedWithClassroomId?: string | null, ownerId: string }; // ownerId for auth check
    ...
    if (mapToUpdate.ownerId !== updates.ownerId) { // Assuming client sends ownerId for this check
        return NextResponse.json({ message: "Unauthorized to update this map" }, { status: 403 });
    }


    const updatedMap = await updateConceptMap(mapId, updates);
    if (!updatedMap) {
      return NextResponse.json({ message: "Concept map not found or update failed" }, { status: 404 });
    }
...
export async function DELETE(request: Request, context: { params: { mapId: string } }) {
  try {
    ...
    if (!mapId || !ownerId) {
      return NextResponse.json({ message: "Map ID and owner ID are required" }, { status: 400 });
    }
