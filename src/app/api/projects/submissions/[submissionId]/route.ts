...
import { getSubmissionById, updateSubmissionStatus } from '@/services/projectSubmissions/projectSubmissionService';
...
export async function PUT(request: Request, context: { params: { submissionId: string } }) {
    try {
        ...
        const { status, analysisError, generatedConceptMapId } = await request.json() as {
        ...
        const updatedSubmission = await updateSubmissionStatus(submissionId, status, analysisError, generatedConceptMapId);
        if (!updatedSubmission) {
            return NextResponse.json({ message: "Submission not found or update failed" }, { status: 404 });
        }
