    ...
    async (classroomId: string, classroomName: string) => {
      try {
        ...
        toast({
          ...
          description: `Classroom "${classroomName}" has been deleted.`,
        });
    ...
    <div className='space-y-6'>
      ...
      {!isLoading && !error && teacherClassrooms.length === 0 && (
        <EmptyState
          icon={BookOpen}
          title={
            deferredSearchTerm ? 'No Classrooms Found' : 'No Classrooms Yet'
          }
          description={
            deferredSearchTerm
              ? 'No classrooms match your search term.'
          ...
          {totalPages > 1 && (
            <CardFooter className='flex items-center justify-between border-t pt-4 mt-6 bg-card'>
              <span className='text-sm text-muted-foreground'>
                Page {currentPage} of {totalPages} ({totalClassrooms}{' '}
                classrooms)
              </span>
