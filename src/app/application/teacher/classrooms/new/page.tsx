...
export default function CreateClassroomPage() {
  ...
  const onSubmit = useCallback(
    async (values: z.infer<typeof classroomFormSchema>) => {
      if (!user || !user.id) {
        toast({
    ...
    <div className='space-y-6'>
      ...
      <Card className='shadow-lg'>
        <CardHeader>
          ...
          <CardDescription>
            Enter the information for your new classroom.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                  ...
                  <FormItem className='flex flex-row items-center justify-between rounded-lg border p-4 shadow-sm'>
                    <div className='space-y-0.5'>
                      <FormLabel className='text-base flex items-center'>
                        <BrainCog className='mr-2 h-5 w-5 text-primary' />{' '}
                        Enable AI Project Analysis for Students
                      </FormLabel>
                      <FormDescription>
                        Allow students in this classroom to submit projects for
