...
describe('useConceptMapAITools', () => {
  ...
  describe('handleSummarizeSelectedNodes', () => {
    const node1 = { id: 'sNode1', text: 'First Summary Node', x: 10, y: 10, type: 'concept' };
    const node2 = { id: 'sNode2', text: 'Second Summary Node', x: 110, y: 10, type: 'concept' };
    const mockSummaryOutput: SummarizeNodesOutput = {
      summary: { text: 'This is a summary of selected nodes.', details: 'Generated by AI.' },
    };
    const mockGetNodePlacement = jest.requireMock('@/lib/layout-utils').getNodePlacement;
    ...
    beforeEach(() => {
      ...
      mockToast.mockClear();
      mockGetNodePlacement.mockReturnValue({ x: 50, y: 100 }); // Mock position for summary node
      mockStore.mapData = { nodes: [node1, node2], edges: [] };
      mockStore.multiSelectedNodeIds = [node1.id, node2.id];
      mockReactFlowInstance.getViewport.mockReturnValue({ x: 0, y: 0, zoom: 1, width: 800, height: 600 });
    });

    test('SUCCESS: should call AI, stage summary node, and show success toast', async () => {
      ...
      expect(mockStore.setStagedMapData).toHaveBeenCalledWith(expect.objectContaining({
        nodes: expect.arrayContaining([
          expect.objectContaining({
            text: mockSummaryOutput.summary.text,
            details: mockSummaryOutput.summary.details,
    ...
    test('FAILURE (NO NODES SELECTED): should show error toast if no nodes are selected', async () => {
      mockStore.multiSelectedNodeIds = []; // No nodes selected
      const { result } = renderHook(() => useConceptMapAITools(false));
    ...
    test('SUCCESS (NO SUMMARY TEXT): should show specific toast if AI returns no summary text', async () => {
      mockAiSummarizeNodes.mockResolvedValue({ summary: { text: '' } }); // AI returns empty summary
      const { result } = renderHook(() => useConceptMapAITools(false));
    ...
    test('FAILURE (AI): should show error toast if AI flow fails', async () => {
      const errorMessage = 'Summarization AI failed';
      mockAiSummarizeNodes.mockRejectedValue(new Error(errorMessage));
      const { result } = renderHook(() => useConceptMapAITools(false));
      ...
      expect(mockStore.setStagedMapData).not.toHaveBeenCalled();
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({
        title: 'Error: Could not complete Summarize Selection',
        description: expect.stringContaining(errorMessage),
      }));
    });

    test('VIEW-ONLY MODE: should show view-only toast and not call AI', async () => {
  ...
  describe('handleRefineSuggestionConfirm', () => {
    const stagedNodeId = 'stagedNode1';
    const refinementInstruction = 'Make it more detailed';
    const mockInitialStagedNode = { id: stagedNodeId, text: 'Original Staged Text', details: 'Original Staged Details', type: 'ai-expanded' };
    const mockRefinedOutput: RefineNodeSuggestionOutput = { refinedText: 'Refined Staged Text', refinedDetails: 'Refined Staged Details' };

    beforeEach(() => {
      mockRefineNodeSuggestionFlow.mockClear();
      mockStore.setStagedMapData.mockClear();
      mockToast.mockClear();
      // Set a default stagedMapData for most tests
      mockStore.stagedMapData = {
        nodes: [mockInitialStagedNode],
        edges: [],
        actionType: 'expandConcept', // Example action type
        originalElementId: 'parent1',
      };
    });

    test('SUCCESS: should call AI, update stagedMapData, and show success toast', async () => {
      mockRefineNodeSuggestionFlow.mockResolvedValue(mockRefinedOutput);
      const { result } = renderHook(() => useConceptMapAITools(false));

      await act(async () => {
        await result.current.handleRefineSuggestionConfirm(stagedNodeId, refinementInstruction);
      });

      expect(mockRefineNodeSuggestionFlow).toHaveBeenCalledWith({
        originalText: mockInitialStagedNode.text,
        originalDetails: mockInitialStagedNode.details,
        refinementInstruction,
      });

      expect(mockStore.setStagedMapData).toHaveBeenCalledWith(
        expect.objectContaining({
          nodes: expect.arrayContaining([
            expect.objectContaining({
              id: stagedNodeId,
              text: mockRefinedOutput.refinedText,
              details: mockRefinedOutput.refinedDetails,
              type: 'ai-refined-staged', // Check for type update
            }),
          ]),
          actionType: 'expandConcept', // Ensure other stagedData properties are preserved
          originalElementId: 'parent1',
        })
      );

      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'Staged Suggestion Updated' })
      );
    });

    test('FAILURE (AI): should show error toast if AI flow fails', async () => {
      const errorMessage = 'Refinement AI failed';
      mockRefineNodeSuggestionFlow.mockRejectedValue(new Error(errorMessage));
      const { result } = renderHook(() => useConceptMapAITools(false));
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'Error: Could not complete Refine Suggestion',
          description: expect.stringContaining(errorMessage),
        })
      );
    });

    test('FAILURE (NO STAGED DATA): should show error toast if stagedMapData is null', async () => {
      mockStore.stagedMapData = null;
      const { result } = renderHook(() => useConceptMapAITools(false));
    ...
    test('FAILURE (NODE NOT IN STAGED DATA): should show error toast if node not found in stagedMapData.nodes', async () => {
      mockStore.stagedMapData = { nodes: [], edges: [], actionType: 'expandConcept' }; // Node not present
      const { result } = renderHook(() => useConceptMapAITools(false));
     ...
     test('VIEW-ONLY MODE: should show view-only toast and not call AI', async () => {
      ...
      // Need to set refineModalInitialData for the function to proceed up to the view-only check
      // However, handleRefineSuggestionConfirm itself doesn't rely on local state refineModalInitialData,
      // it relies on nodeId and stagedMapData from the store.
      // The view-only check in callAIWithStandardFeedback will be triggered.
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'View Only Mode',
          description: 'Refine Suggestion is disabled.',
        })
      );
    });
  });

  describe('handleConceptsExtracted (testing callAIWithStandardFeedback indirectly)', () => {
    const testText = 'Sample text for extraction.';
    const mockExtractedConceptsData = {
      concepts: [
        { concept: 'Concept 1', context: 'Sample' },
        { concept: 'Concept 2' },
      ],
    };

    beforeEach(() => {
        mockAiExtractConcepts.mockClear();
        mockStore.setAiExtractedConcepts.mockClear();
        mockToast.mockClear();
        mockStore.setAiProcessingNodeId.mockClear();
    });

    test('SUCCESS CASE: should call AI flow, execute onSuccess, show success toast, and manage processing ID', async () => {
      mockAiExtractConcepts.mockResolvedValue(mockExtractedConceptsData);
      const { result } = renderHook(() => useConceptMapAITools(false));

      let successReturn: boolean | undefined;
      await act(async () => {
        successReturn = await result.current.handleConceptsExtracted(testText);
      });

      // Verify AI flow call
      expect(mockAiExtractConcepts).toHaveBeenCalledWith({ text: testText });

      // Verify onSuccess callback execution (specific to handleConceptsExtracted)
      expect(mockStore.setAiExtractedConcepts).toHaveBeenCalledWith(
        mockExtractedConceptsData.concepts
      );

      // Verify success toast
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'Extract Concepts Successful!',
          description: `${mockExtractedConceptsData.concepts.length} concepts found. View in AI Panel.`,
        })
      );

      // Verify return value
      expect(successReturn).toBe(true);

      // Verify processing ID management
      expect(mockStore.setAiProcessingNodeId).toHaveBeenCalledTimes(2);
      expect(mockStore.setAiProcessingNodeId).toHaveBeenNthCalledWith(1, 'Extract Concepts');
      expect(mockStore.setAiProcessingNodeId).toHaveBeenNthCalledWith(2, null);
    });

    test('FAILURE CASE: should show error toast and not execute onSuccess on AI flow failure', async () => {
      const errorMessage = 'AI extraction failed intentionally';
      const errorDetails = 'Genkit detail: something specific went wrong.';
      const mockError = new Error(errorMessage);
      (mockError as any).details = errorDetails; // Simulate Genkit error structure
      mockAiExtractConcepts.mockRejectedValue(mockError);
      ...
      expect(mockAiExtractConcepts).toHaveBeenCalledWith({ text: testText });
      expect(mockStore.setAiExtractedConcepts).not.toHaveBeenCalled(); // onSuccess should not be called

      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'Error: Could not complete Extract Concepts',
          description: expect.stringContaining(`Details: ${errorDetails}`), // Check for error.details
          variant: 'destructive',
        })
      );
      expect(successReturn).toBe(false);
      expect(mockStore.setAiProcessingNodeId).toHaveBeenCalledTimes(2); // Still called for start and finally
    });

    test('VIEW-ONLY MODE: should not call AI flow and show view-only toast', async () => {
      const { result } = renderHook(() => useConceptMapAITools(true)); // isViewOnlyMode = true
      ...
      expect(successReturn).toBe(false); // Or null depending on strictness, but false for boolean check
      expect(mockStore.setAiProcessingNodeId).not.toHaveBeenCalled();
    });

    test('FAILURE CASE with custom onError that suppresses toast: should call custom onError and not show default toast', async () => {
      const errorMessage = 'AI failed, but custom handler will manage.';
      mockAiExtractConcepts.mockRejectedValue(new Error(errorMessage));

      const customOnError = jest.fn().mockReturnValue(true); // Suppress default toast

      // To test this, we need to call callAIWithStandardFeedback directly or modify an existing handler
      // For simplicity in this example, let's assume we could pass onError to handleConceptsExtracted,
      // or we test callAIWithStandardFeedback more directly.
      // Since handleConceptsExtracted doesn't take onError, we'll note this for direct callAI... tests.
      // This test highlights a scenario for callAIWithStandardFeedback itself.
      // For now, we can ensure the basic error path of handleConceptsExtracted works.

      // Re-testing the normal error path for handleConceptsExtracted as it doesn't use custom onError
      const { result } = renderHook(() => useConceptMapAITools(false));
      await act(async () => {
        await result.current.handleConceptsExtracted(testText);
      });
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          variant: 'destructive',
  ...
  describe('addExtractedConceptsToMap', () => {
    ...
    test('should add selected concepts to the map and show toast', () => {
      ...
      expect(
        mockStore.removeExtractedConceptsFromSuggestions
      ).toHaveBeenCalledWith(mockConceptsToAdd.map((c) => c.concept));
    });
  ...
  // Tests for handleConceptExpanded
  describe('handleConceptExpanded', () => {
    const mockParentNode = { id: 'parent1', text: 'Parent Concept', x: 50, y: 50, type: 'concept' };
    ...
    const mockExpansionOutput: ExpandConceptOutput = {
      expandedIdeas: [{ text: 'Child Idea 1', reasoning: 'Reason 1', relationLabel: 'relates to' }],
    };
    const mockGetNodePlacement = jest.requireMock('@/lib/layout-utils').getNodePlacement;
    ...
    beforeEach(() => {
      ...
      mockToast.mockClear();
      mockStore.setAiProcessingNodeId.mockClear();
      mockGetNodePlacement.mockReturnValue({ x: 100, y: 150 }); // Default mock position
      // Setup initial state for conceptToExpandDetails via the openExpandConceptModal or directly for tests
      // For these tests, we'll assume conceptToExpandDetails is properly set before calling handleConceptExpanded
      // This is typically done by calling openExpandConceptModal first, which sets local state in the hook.
      // To simulate this for direct testing of handleConceptExpanded, we'd need to manipulate local state or mock it.
      // Alternative: call openExpandConceptModal then handleConceptExpanded in the test.
    });

    test('should show error and return null if view-only mode', async () => {
      const { result } = renderHook(() => useConceptMapAITools(true)); // isViewOnlyMode = true
      act(() => { // Simulate that openExpandConceptModal was called and set the details
          result.current.conceptToExpandDetails = { id: 'parent1', text: 'Parent Concept', node: mockParentNode as any };
      });
      const returnValue = await act(async () => result.current.handleConceptExpanded(mockExpandInput));
    ...
    test('should show error and return null if conceptToExpandDetails.id is null', async () => {
      const { result } = renderHook(() => useConceptMapAITools(false));
       act(() => {
          result.current.conceptToExpandDetails = { id: null, text: 'Parent Concept' };
      });
      const returnValue = await act(async () => result.current.handleConceptExpanded(mockExpandInput));
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ description: 'Cannot expand concept without a source node on the map.' }));
      expect(returnValue).toBeNull();
      expect(mockAiExpandConcept).not.toHaveBeenCalled();
    });

    test('SUCCESS: should call AI, stage results, and show success toast', async () => {
      mockAiExpandConcept.mockResolvedValue(mockExpansionOutput);
      mockStore.mapData = { nodes: [mockParentNode], edges: [] }; // Ensure parentNode is in mapData

      const { result } = renderHook(() => useConceptMapAITools(false));
      act(() => {
        result.current.conceptToExpandDetails = { id: mockParentNode.id, text: mockParentNode.text, node: mockParentNode as any };
      });

      const success = await act(async () => result.current.handleConceptExpanded(mockExpandInput));

      expect(mockAiExpandConcept).toHaveBeenCalledWith(mockExpandInput);
      expect(mockStore.setStagedMapData).toHaveBeenCalledWith(expect.objectContaining({
        nodes: expect.arrayContaining([expect.objectContaining({ text: 'Child Idea 1', type: 'ai-expanded' })]),
        edges: expect.arrayContaining([expect.objectContaining({ source: mockParentNode.id, label: 'relates to' })]),
        actionType: 'expandConcept',
        originalElementId: mockParentNode.id,
      }));
      expect(mockGetNodePlacement).toHaveBeenCalled();
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ title: 'AI Suggestions Ready' }));
      expect(success).toBe(true); // Assuming callAIWithStandardFeedback returns true on non-null output
    });

    test('SUCCESS (NO IDEAS): should show specific toast if AI returns no expanded ideas', async () => {
      mockAiExpandConcept.mockResolvedValue({ expandedIdeas: [] });
      mockStore.mapData = { nodes: [mockParentNode], edges: [] };
      const { result } = renderHook(() => useConceptMapAITools(false));
       act(() => {
        result.current.conceptToExpandDetails = { id: mockParentNode.id, text: mockParentNode.text, node: mockParentNode as any };
      });
      await act(async () => result.current.handleConceptExpanded(mockExpandInput));
     ...
     test('FAILURE (PARENT NOT FOUND): should show error if parent node not found after AI call (edge case)', async () => {
      mockAiExpandConcept.mockResolvedValue(mockExpansionOutput);
      mockStore.mapData = { nodes: [], edges: [] }; // Parent node disappears
      const { result } = renderHook(() => useConceptMapAITools(false));
      act(() => {
        result.current.conceptToExpandDetails = { id: 'nonExistentParent', text: 'Text', node: undefined };
      });

      // Simulate parentNodeId correctly set for the AI call part
      result.current.conceptToExpandDetails = { id: 'parent1', text: 'Parent Concept', node: {id: 'parent1'} as any };


      await act(async () => result.current.handleConceptExpanded(mockExpandInput));

      // This toast comes from within the onSuccess of callAIWithStandardFeedback
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({
        title: 'Error',
  ...
  describe('openSuggestRelationsModal', () => {
    ...
    test('should use selected node and its neighbors if single node selected', () => {
      ...
      expect(result.current.conceptsForRelationSuggestion).toContain(
        mockNode1.text
      );
      expect(result.current.conceptsForRelationSuggestion).toContain(
        mockNode2.text
  ...
  describe('handleRelationsSuggested', () => {
    ...
    const mockSuggestedRelationsData: SuggestRelationsOutput = [
      {
        ...
        reason: 'They are often discussed together.',
      },
    ...
    test('should call AI flow and update store on successful suggestion', async () => {
      ...
      expect(mockAiSuggestRelations).toHaveBeenCalledWith({
        concepts: testConcepts,
      });
      expect(mockStore.setAiSuggestedRelations).toHaveBeenCalledWith(
        mockSuggestedRelationsData
      );
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'Suggest Relations Successful!' })
      );
      expect(success).toBe(true);
      expect(mockStore.setAiProcessingNodeId).toHaveBeenCalledWith(
        'Suggest Relations'
      );
  ...
  describe('addSuggestedRelationsToMap', () => {
    ...
    beforeEach(() => {
      mockStore.mapData = {
        nodes: [mockNodeA as any, mockNodeB as any],
        edges: [],
      };
    ...
    test('should add relations, creating new nodes if necessary, and show toast', () => {
      ...
      const edgeToNewNode = addEdgeCalls.find(
        (call) =>
          call[0].source === mockNodeB.id && call[0].label === 'leads to'
      );
      ...
      expect(
        mockStore.removeSuggestedRelationsFromSuggestions
      ).toHaveBeenCalledWith(mockRelationsToAdd);
    });
    ...
    test('should not create duplicate edges if relation already exists', () => {
      ...
      const { result } = renderHook(() => useConceptMapAITools(false));
      act(() =>
        result.current.addSuggestedRelationsToMap(relationsToTest as any)
      );

      expect(mockStore.addEdge).not.toHaveBeenCalled();
      const toastCallsWithTitle = mockToast.mock.calls.filter(
        (call) => call[0].title === 'Relations Added'
      );
  ...
  describe('openExpandConceptModal', () => {
    ...
    test('should open modal and set concept details from selected node', () => {
      ...
      expect(result.current.conceptToExpandDetails).toEqual({
        id: 'n1',
        text: 'Expandable Node',
        node: mockNode1,
      });
  ...
  describe('handleConceptExpanded', () => {
    ...
    const mockExpandInput: ExpandConceptInput = {
    ...
    beforeEach(() => {
      ...
      mockToast.mockClear();
      mockStore.setAiProcessingNodeId.mockClear();
      mockStore.conceptToExpandDetails = {
        id: mockParentNode.id,
        text: mockParentNode.text,
        node: mockParentNode as any,
      };
    ...
    test('should call AI, stage results, and clear preview on successful expansion', async () => {
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'AI Suggestions Ready' })
      );
      expect(mockStore.setAiProcessingNodeId).toHaveBeenCalledWith(
        mockParentNode.id
      );
  ...
  describe('handleSummarizeSelectedNodes', () => {
    ...
    test('should summarize selected nodes and stage the result', async () => {
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'AI Summary Created!' })
      );
    });
  ...
  describe('openQuickClusterModal', () => {
    ...
    test('should show toast and not open modal if not enough nodes selected', () => {
      ...
      expect(mockToast).toHaveBeenCalledWith({
        ...
        description:
          'Please select at least two nodes to form a quick cluster.',
        variant: 'destructive',
      });
    });
  ...
  describe('handleMiniToolbarQuickExpand', () => {
    ...
    test('should quick expand node and stage result', async () => {
      ...
      expect(mockAiExpandConcept).toHaveBeenCalledWith(
        expect.objectContaining({
      ...
      expect(mockStore.setStagedMapData).toHaveBeenCalledWith(
        expect.objectContaining({
          nodes: expect.arrayContaining([
            expect.objectContaining({ text: 'Quick Idea' }),
          ]),
          edges: expect.arrayContaining([
            expect.objectContaining({
              source: sourceNodeId,
      ...
      expect(mockStore.setConceptExpansionPreview).toHaveBeenCalledWith(null);
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'AI Suggestion Ready' })
      );
    });
  ...
  describe('handleMiniToolbarRewriteConcise', () => {
    ...
    const mockNodeToRewrite = {
      id: nodeIdToRewrite,
      text: 'Some long text to make concise',
  ...
  describe('handleQuestionAnswered', () => {
    ...
    const mockAnswer: AskQuestionAboutNodeOutput = {
      answer: 'This node is about important things.',
    };
    ...
    beforeEach(() => {
      ...
      mockToast.mockClear();
      mockStore.mapData = {
        nodes: [{ ...nodeCtx, type: 'concept' } as any],
        edges: [],
      };
    });
  ...
  describe('openAskQuestionAboutEdgeModal', () => {
    ...
    test('should open modal and set edge context', () => {
      ...
      expect(result.current.edgeQuestionContext).toEqual({
        ...
        sourceNodeDetails: 'Source Details',
        targetNodeId: 't1',
        targetNodeText: 'Target Node',
        targetNodeDetails: 'Target Details',
        edgeId: 'edge1',
        edgeLabel: 'connects',
        userQuestion: '',
      });
    ...
    test('should show error if edge not found', () => {
      const { result } = renderHook(() => useConceptMapAITools(false));
      act(() =>
        result.current.openAskQuestionAboutEdgeModal('nonExistentEdge')
      );
  ...
  describe('handleAskQuestionAboutEdge', () => {
    ...
    const edgeCtx: AskQuestionAboutEdgeInput = {
      ...
      sourceNodeDetails: 'SDetails',
      targetNodeId: 't1',
      targetNodeText: 'Target',
      targetNodeDetails: 'TDetails',
      edgeId: 'edge1',
      edgeLabel: 'connects',
      userQuestion: question,
    };
    const mockEdgeAnswer: AskQuestionAboutEdgeOutput = {
      answer: 'This connection is important because...',
    };
    ...
    test('should call AI flow and set answer state on success', async () => {
      ...
      act(() => {
        result.current.edgeQuestionContext = edgeCtx;
      });
    ...
    test('should set error message in answer state on AI failure', async () => {
      ...
      rerender();
      expect(result.current.edgeQuestionAnswer).toBe(
    ...
    test('should show error if edgeQuestionContext is null', async () => {
      const { result } = renderHook(() => useConceptMapAITools(false));
      act(() => {
        result.current.edgeQuestionContext = null;
      });
  ...
  describe('openAskQuestionAboutMapContextModal', () => {
    ...
    test('should open modal and clear previous answer', () => {
      const { result } = renderHook(() => useConceptMapAITools(false));
      act(() => {
        result.current.mapContextQuestionAnswer = 'Previous answer';
      });

      act(() => {
        result.current.openAskQuestionAboutMapContextModal();
      });
  ...
  describe('handleAskQuestionAboutMapContext', () => {
    ...
    const mockMapAnswer: AskQuestionAboutMapContextOutput = {
      answer: 'The map is about project architecture.',
    };
    const node1 = {
      id: 'n1',
      text: 'Node 1',
    ...
    test('should call AI flow and set answer on success', async () => {
      ...
      expect(mockAskQuestionAboutMapContextFlow).toHaveBeenCalledWith({
        nodes: [
          { id: 'n1', text: 'Node 1', type: 'generic', details: 'Detail 1' },
        ],
        edges: [{ source: 'n1', target: 'n2', label: 'connects' }],
        userQuestion: question,
        mapName: 'My Test Map',
      });
      expect(result.current.mapContextQuestionAnswer).toBe(
        mockMapAnswer.answer
      );
  ...
  describe('handleRewriteNodeContentConfirm', () => {
    ...
    test('should update node in store and show toast', async () => {
      const { result } = renderHook(() => useConceptMapAITools(false));
      await act(async () => {
        result.current.handleRewriteNodeContentConfirm(
          nodeId,
          newText,
          newDetails,
          'formal'
      ...
      expect(mockStore.addDebugLog).toHaveBeenCalledWith(
        `[AITools] Applying rewritten content for node ${nodeId}. Tone: formal`
      );
  ...
  describe('handleSummarizeMap', () => {
    ...
    test('should call AI flow, set summary result, and open modal on success', async () => {
      ...
      expect(mockGenerateMapSummaryFlow).toHaveBeenCalledWith({
        nodes: mockNodes,
        edges: mockEdges,
      });
    ...
    test('should set summary result to null on AI failure', async () => {
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'Error: Could not complete Summarize Map',
        })
      );
    });
  ...
  describe('fetchAndSetEdgeLabelSuggestions', () => {
    ...
    const mockLabelsOutput: SuggestEdgeLabelOutput = {
      suggestedLabels: ['suggested label 1', 'suggested label 2'],
    };
    ...
    test('should call AI flow and set edgeLabelSuggestions state', async () => {
      ...
      await act(async () => {
        await result.current.fetchAndSetEdgeLabelSuggestions(
          edgeId,
          sourceNode.id,
          targetNode.id,
          'old label'
      ...
      expect(result.current.edgeLabelSuggestions).toEqual({
        edgeId,
        labels: mockLabelsOutput.suggestedLabels,
      });
    });
  ...
  describe('fetchAIChildTextSuggestions', () => {
    ...
    const mockSuggestionsOutput: SuggestQuickChildTextsOutput = {
      suggestedChildTexts: ['New Child 1', 'New Child 2'],
    };
    ...
    test('should call AI and update suggestions state', async () => {
      ...
      expect(result.current.aiChildTextSuggestions).toEqual(
        mockSuggestionsOutput.suggestedChildTexts
      );
    ...
    test('should handle AI flow failure', async () => {
      mockSuggestQuickChildTextsFlow.mockRejectedValue(
        new Error('Child suggestion failed')
      );
      const { result } = renderHook(() => useConceptMapAITools(false));
      await act(async () => {
        await result.current.fetchAIChildTextSuggestions(parentNodeData);
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'Error: Could not complete Suggest Child Node Texts',
        })
      );
      expect(result.current.isLoadingAiChildTexts).toBe(false);
    });
  ...
  describe('handleSuggestIntermediateNodeRequest', () => {
    ...
    test('should call AI, stage results, and show toast on success', async () => {
      ...
      await act(async () => {
        await result.current.handleSuggestIntermediateNodeRequest(
          edge.id,
          sourceNode.id,
          targetNode.id
        );
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'Intermediate Node Suggested' })
      );
    });
  ...
  describe('handleAiTidyUpSelection', () => {
    ...
    test('should show error if less than 2 nodes selected', async () => {
      ...
      const { result } = renderHook(() => useConceptMapAITools(false));
      await act(async () => {
        await result.current.handleAiTidyUpSelection();
      });
    ...
    test('should set ghost preview for layout-only changes', async () => {
      ...
      expect(mockStore.setStagedMapData).not.toHaveBeenCalled();
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'Layout Preview Ready' })
      );
    });
    ...
    test('should stage results if new parent node is suggested', async () => {
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'AI Tidy Up Suggestion Ready' })
      );
    });
  });
  ...
  describe('handleSuggestMapImprovements', () => {
    ...
    beforeEach(() => {
      ...
      mockToast.mockClear();
      mockStore.mapData = {
        nodes: mockNodesInput as any[],
        edges: mockEdgesInput as any[],
      };
    });
    ...
    test('should call AI flow and set structural suggestions on success', async () => {
      ...
      expect(mockSuggestMapImprovementsFlow).toHaveBeenCalledWith({
        nodes: mockNodesInput,
        edges: mockEdgesInput,
      });
      expect(mockStore.setStructuralSuggestions).toHaveBeenCalledWith(
        mockImprovementsOutput
      );
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'Improvement Suggestions Ready!' })
      );
    });

    test('should handle AI flow failure', async () => {
      mockSuggestMapImprovementsFlow.mockRejectedValue(
        new Error('Improvement suggestion failed')
      );
      const { result } = renderHook(() => useConceptMapAITools(false));
      await act(async () => {
        await result.current.handleSuggestMapImprovements();
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          title: 'Error: Could not complete Suggest Map Improvements',
        })
      );
    });
  });
  ...
  describe('handleDagreLayoutSelection', () => {
    ...
    const mockLayoutPositions = [
      { id: 'dagre1', x: 15, y: 15 },
      { id: 'dagre2', x: 205, y: 15 },
    ];
    ...
    test('should apply Dagre layout and set ghost preview', async () => {
      ...
      const { result } = renderHook(() => useConceptMapAITools(false));
      await act(async () => {
        await result.current.handleDagreLayoutSelection();
      });
  ...
  describe('openRefineSuggestionModal', () => {
    ...
    test('should not open modal if preview node not found', () => {
      const { result } = renderHook(() => useConceptMapAITools(false));
      act(() => {
        result.current.openRefineSuggestionModal(
          'nonexistentPreview',
          parentId
        );
      });
  ...
  describe('handleRefineSuggestionConfirm', () => {
    ...
    test('should call AI flow and update preview node on success', async () => {
      ...
      await act(async () => {
        await result.current.handleRefineSuggestionConfirm(
          nodeIdToRefine,
          refinementInstruction
        );
      });
      ...
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({ title: 'Suggestion Refined!' })
      );
    });
