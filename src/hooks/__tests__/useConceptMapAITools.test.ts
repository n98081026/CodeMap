...
describe('useConceptMapAITools', () => {
  ...
  describe('handleSummarizeSelectedNodes', () => {
    ...
    const mockSummaryOutput = {
      summary: {
        text: 'This is a summary of selected nodes.',
        details: 'Generated by AI.',
      },
    };
    const mockGetNodePlacement =
      jest.requireMock('@/lib/layout-utils').getNodePlacement;
    ...
    beforeEach(() => {
      ...
      mockReactFlowInstance.getViewport.mockReturnValue({
        x: 0,
        y: 0,
        zoom: 1,
        width: 800,
  ...
  describe('handleRefineSuggestionConfirm', () => {
    ...
    const mockInitialStagedNode = {
      id: stagedNodeId,
      text: 'Original Staged Text',
      details: 'Original Staged Details',
      type: 'ai-expanded',
    };
    const mockRefinedOutput = {
      refinedText: 'Refined Staged Text',
      refinedDetails: 'Refined Staged Details',
    };
  ...
  describe('handleConceptsExtracted (testing callAIWithStandardFeedback indirectly)', () => {
    ...
    test('SUCCESS CASE: should call AI flow, execute onSuccess, show success toast, and manage processing ID', async () => {
      ...
      expect(mockStore.setAiProcessingNodeId).toHaveBeenNthCalledWith(
        1,
        'Extract Concepts'
      );
  ...
  describe('handleConceptExpanded', () => {
    ...
    test('should show error and return null if view-only mode', async () => {
      const { result } = renderHook(() => useConceptMapAITools(true)); // isViewOnlyMode = true
      act(() => {
        // Simulate that openExpandConceptModal was called and set the details
        result.current.conceptToExpandDetails = {
          id: 'parent1',
          text: 'Parent Concept',
    ...
    const returnValue = await act(async () => {
        if (result.current.handleConceptExpanded) {
          return await result.current.handleConceptExpanded(mockExpandInput);
        }
        return null;
      });
      expect(mockToast).toHaveBeenCalledWith(
    ...
    test('should show error and return null if conceptToExpandDetails.id is null', async () => {
      ...
      const returnValue = await act(async () =>
        result.current.handleConceptExpanded(mockExpandInput)
      );
      expect(mockToast).toHaveBeenCalledWith(
        expect.objectContaining({
          description:
  ...
  describe('handleAiTidyUpSelection', () => {
    ...
    const mockTidyOutputLayoutOnly = {
      newPositions: [
        { id: 'tidy1', x: 15, y: 15 },
        { id: 'tidy2', x: 205, y: 15 },
      ],
    };
    const mockTidyOutputWithParent = {
      newPositions: [
        { id: 'tidy1', x: 50, y: 50 },
        { id: 'tidy2', x: 50, y: 120 },
      ],
