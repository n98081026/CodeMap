...
import { EnhancedLayoutEngine, type LayoutOptions } from '@/components/concept-map/enhanced-layout-engine';
...
export function useEnhancedVisualEffects(isViewOnlyMode: boolean = false) {
   ...
   * 節點出現動畫
   */
  const animateNodeAppearance = useCallback((nodeId: string, animationType: 'fadeIn' | 'scaleUp' | 'bounce' = 'fadeIn') => {
    if (isViewOnlyMode) return;
   ...
   * 節點點擊效果
   */
  const triggerNodeClickEffect = useCallback((nodeId: string) => {
    if (isViewOnlyMode) return;
   ...
   * 節點懸停效果
   */
  const triggerNodeHoverEffect = useCallback((nodeId: string, isHovering: boolean) => {
    if (isViewOnlyMode) return;
   ...
   * 執行增強佈局動畫
   */
  const executeEnhancedLayout = useCallback(async (layoutType: LayoutOptions['type']) => {
   ...
   * AI 生成內容動畫
   */
  const animateAIGeneration = useCallback((position: { x: number; y: number }) => {
    if (isViewOnlyMode) return;
   ...
   * 批量動畫節點
   */
  const batchAnimateNodes = useCallback((nodeIds: string[], animationType: 'fadeIn' | 'scaleUp' | 'bounce' = 'fadeIn') => {
    if (isViewOnlyMode) return;
  ...
  return {
    ...
    // 佈局選項
    availableLayouts: ['hierarchical', 'circular', 'force', 'grid', 'organic'] as LayoutOptions['type'][],
  };
}