...
export function useWhimsicalAITools(isViewOnlyMode: boolean) {
   ...
   * Whimsical 風格的智能概念提取
   */
  const handleWhimsicalExtractConcepts = useCallback(async (
    text: string,
    userGoals?: string,
    difficultyLevel?: 'beginner' | 'intermediate' | 'advanced'
  ) => {
    if (isViewOnlyMode) {
      toast({ title: "檢視模式", description: "AI 功能已停用", variant: "default" });
      return;
    }
    ...
    try {
      ...
      const mapContext = mapData.nodes.length > 0 
      ...
      if (result.concepts && result.concepts.length > 0) {
        ...
        result.concepts.forEach((conceptItem, index) => {
          ...
          const newNode: ConceptMapNode = {
            ...
            details: [
              conceptItem.context,
              conceptItem.pedagogicalNote && `💡 學習提示: ${conceptItem.pedagogicalNote}`,
