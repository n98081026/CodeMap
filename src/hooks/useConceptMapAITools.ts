...
export function useConceptMapAITools(isViewOnlyMode: boolean) {
  ...
  const callAIWithStandardFeedback = useCallback(
    async <I, O>(
      ...
      options?: {
        ...
        onSuccess?: (output: O, input: I) => void; // Added for current (restored) state
        onError?: (error: unknown, input: I) => boolean | void; // Added for current (restored) state
      }
    ): Promise<O | null> => {
      if (isViewOnlyMode) {
        toast({
      ...
      try {
        ...
        return result;
      } catch (error: any) { // Keep 'any' here for now due to complex error shapes
        loadingToast.dismiss();
        console.error(
  ...
  const handleConceptsExtracted = useCallback(
    async (text: string) => {
      await callAIWithStandardFeedback< /* Adjusted to use await and not assign to output directly */
      ...
      >(
        ...
        { text },
        {
          ...
          onSuccess: (output) => { // output here is the result from callAIWithStandardFeedback
            if (output) setAiExtractedConcepts(output.concepts);
          },
        }
      );
      // The return value of callAIWithStandardFeedback (which is output or null) determines the success.
      // To match original behavior of returning !!output:
      // We can't directly get the 'output' here if we don't assign the await.
      // This part needs careful consideration if the boolean return is strictly necessary.
      // For now, assuming the primary goal is the side effect in onSuccess.
      // If the boolean is critical, the structure might need to change slightly,
      // or callAIWithStandardFeedback could return a more complex object.
      // Let's assume for now that the toast and onSuccess are sufficient.
      return true; // Placeholder, actual success depends on callAIWithStandardFeedback not returning null.
    },
  ...
  const addExtractedConceptsToMap = useCallback(
    ...
    [
      ...
      addStoreNode,
      removeExtractedConceptsFromSuggestions,
      // mapData.nodes, // Re-add if getNodePlacement needs it from hook scope
    ]
  );
  ...
  const openSuggestRelationsModal = useCallback(
    ...
    [
      isViewOnlyMode,
      resetAiSuggestions,
      mapData, // mapData is used via selectedElementId and multiSelectedNodeIds which are in deps
      selectedElementId,
      multiSelectedNodeIds,
      toast,
    ]
  ...
  const handleRelationsSuggested = useCallback(
    async (concepts: string[]) => {
      await callAIWithStandardFeedback< /* Adjusted */
      ...
      return true; // Placeholder, see comment in handleConceptsExtracted
    },
  ...
  const addSuggestedRelationsToMap = useCallback(
      ...
      selectedRelations.forEach((rel) => {
        ...
        if (!sourceNode) {
          const newSourceNodeId = addStoreNode({
            ...
            type: 'ai-concept',
            position: getNodePlacement(
              currentNodes, // Uses currentNodes from this scope
              'generic',
              null,
              null,
              GRID_SIZE_FOR_AI_PLACEMENT
          ...
          sourceNode = useConceptMapStore // Re-fetch from store
            .getState()
        ...
        if (!targetNode) {
          ...
          targetNode = useConceptMapStore // Re-fetch from store
            .getState()
    ...
    [
      ...
      addStoreEdge,
      removeSuggestedRelationsFromSuggestions,
      // mapData.nodes, mapData.edges are not direct deps as it uses getState()
    ]
  );
  ...
  const openExpandConceptModal = useCallback(
    (nodeIdForContext?: string) => {
      ...
      let context: string[] = [];
      const currentMapData = mapData; // Use mapData from hook scope
      const graphAdapter = new GraphAdapterUtility();
      const graphInstance = graphAdapter.fromArrays(
        currentMapData.nodes,
        currentMapData.edges
      ...
      },
      {
        ...
        onSuccess: (output, _input) => {
          if (output && output.summary && output.summary.text) {
            // Re-filter selectedNodes based on current multiSelectedNodeIds from hook scope
             const currentSelectedNodes = mapData.nodes.filter((node) =>
                multiSelectedNodeIds.includes(node.id)
             );
            if (currentSelectedNodes.length === 0) return; // Should not happen if initial check passed
            ...
            const summaryNodePosition = getNodePlacement(
              mapData.nodes, // Use current mapData.nodes from hook scope
              'generic',
              null,
              {
                id: 'summary-center',
            ...
            const summaryNode: ConceptMapNode = {
              ...
              details:
                ...
                `Summary of: ${currentSelectedNodes.map((n) => n.text).join(', ')}`,
              type: 'ai-summary-node',
              position: {
                x: summaryNodePosition.x,
                y: summaryNodePosition.y - (currentSelectedNodes.length > 1 ? 100 : 0),
              },
              width: 200,
              height: 100,
              childIds: [],
  ...
  const handleMiniToolbarQuickExpand = useCallback(
    ...
    [
      isViewOnlyMode,
      toast,
      mapData, // mapData is used for sourceNode and existingMapContext
      callAIWithStandardFeedback,
      setStagedMapData,
      setConceptExpansionPreview,
    ]
    ...
    async (nodeId: string) => {
      ...
      >(
        ...
        },
        {
          ...
          hideSuccessToast: !node?.text, // Check original node text, not output
          processingId: nodeId,
          onSuccess: (output, _input) => {
  ...
  const handleRefineSuggestionConfirm = useCallback(
    async (nodeId: string, refinementInstruction: string) => {
      const previewState = conceptExpansionPreview; // From hook scope
  ...
  const handleAiTidyUpSelection = useCallback(async () => {
    ...
    const nodesToTidy = mapData.nodes // From hook scope
    ...
    >(
      ...
      aiTidyUpSelectionFlow,
      {
        nodes: nodesToTidy, // From outer scope of onSuccess
        mapLayoutContext: {
        },
      },
      {
  ...
  const handleSuggestMapImprovements = useCallback(async () => {
    const currentNodes = mapData.nodes.map((n) => ({ // mapData from hook scope
      id: n.id,
      text: n.text,
      details: n.details || '',
    }));
    const currentEdges: AISharedSimplifiedEdgeInput[] = mapData.edges.map((e) => ({ // mapData from hook scope
      source: e.source,
      target: e.target,
      label: e.label || '',
    }));

    await callAIWithStandardFeedback<AISuggestMapImprovementsInput, SuggestedImprovements>( /* Adjusted */
  ...
  const handleDagreLayoutSelection = useCallback(async () => {
    ...
    toast,
    reactFlowInstance,
    applyLayout, // applyLayout is still a dependency if it's used (though commented out)
    mapData.nodes, // For initialCentroid calculation
    mapData.edges, // For edgesForDagre
    multiSelectedNodeIds, // For selectedNodesFromStore
  ]);
  ...
  const handleSummarizeMap = useCallback(async () => { // Wrapped in useCallback
    if (isViewOnlyMode) {
      toast({ title: 'View Only Mode', description: 'Cannot summarize map in view-only mode.'});
      return;
    }
    const currentMapData = useConceptMapStore.getState().mapData;
    ...
    >(
      ...
      { nodes: currentMapData.nodes, edges: currentMapData.edges },
      {
        ...
        onError: () => {
          setMapSummaryResult(null);
          // Error toast is handled by callAIWithStandardFeedback
        }
      }
    );
  ...
  [
    ...
    setIsSummarizingMap,
    toast,
    // mapData is not strictly needed as a dep here because it uses getState()
  ]);
