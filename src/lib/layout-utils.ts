...
export function getNodePlacement(
  ...
  if (type === 'child' && parentNode && typeof parentNode.x === 'number' && typeof parentNode.y === 'number') {
    ...
    if (typeof directionOrIndex === 'string') {
        ...
        const childIndex = typeof directionOrIndex === 'number' ? directionOrIndex : existingNodes.filter(n => n.parentNode === parentNode.id).length;
        const totalSiblings = typeof totalSiblingsForSpiral === 'number' && totalSiblingsForSpiral > 0 ? totalSiblingsForSpiral : Math.max(1, childIndex + 1);
        const angleStep = (2 * Math.PI) / Math.max(1, totalSiblings > 3 ? 6 : totalSiblings); // Distribute more widely for more items
        ...
        if (childIndex === 0 && totalSiblings === 1) {
            ...
            finalX = parentNode.x + (parentW / 2) - (defaultNodeWidth / 2) + radius * Math.cos(angle);
            finalY = parentNode.y + (parentH / 2) - (defaultNodeHeight / 2) + radius * Math.sin(angle);
        }
    }
    ...
    finalX = selectedNode.x + (selectedNode.width || defaultNodeWidth) * 0.5 + 30 + (Math.random() * 40 - 20);
    finalY = selectedNode.y + (selectedNode.height || defaultNodeHeight) * 0.5 + 30 + (Math.random() * 40 - 20);
  }
   else { // Generic placement if no context
    let maxX = 0;
    let maxY = 0;
...
export function calculateSnappedPositionAndLines(
  ...
  effectiveNodesToSnapAgainst.forEach(otherNode => {
    ...
    if (otherNode.width == null || otherNode.height == null || !otherNode.positionAbsolute) return;
