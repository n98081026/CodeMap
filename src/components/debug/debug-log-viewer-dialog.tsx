...
export function DebugLogViewerDialog({ isOpen, onOpenChange }: DebugLogViewerDialogProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl h-[70vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Real-time logs from the concept map data manager. Most recent logs at the bottom.
          </DialogDescription>
        </DialogHeader>
        <ScrollArea className="flex-grow border rounded-md p-2 bg-muted/30 text-xs font-mono">
          {debugLogs.length === 0 ? (
            <p className="text-center text-muted-foreground py-4">No logs yet. Interact with the map or change routes.</p>
          ) : (
            debugLogs.map((log, index) => (
              <div key={index} className="whitespace-pre-wrap break-all py-0.5 border-b border-border/50 last:border-b-0">
                {log}
              </div>
            ))
          )}
        ...
        <DialogFooter className="mt-auto pt-4 border-t">
          ...
          <DialogClose asChild>
            <Button type="button" variant="default" size="sm">Close</Button>
          </DialogClose>
        </DialogFooter>
      </DialogContent>
    </Dialog>
