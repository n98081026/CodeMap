...
const MAX_FILE_SIZE_MB_FROM_ENV = parseInt(
  process.env.NEXT_PUBLIC_MAX_PROJECT_FILE_SIZE_MB || '10',
  10
);
...
const projectUploadSchema = z.object({
  projectFile: z
    .custom<FileList>(
      ...
      'Project archive file is required.'
    )
    .refine(
      (files) => files && files.length === 1,
      'Exactly one project archive file is required.'
    )
    .refine(
    ...
    .refine((files) => {
      ...
      if (fileExtension && acceptedExtensions.includes(fileExtension))
        return true;
      if (
        (file.name.endsWith('.tar.gz') || file.name.endsWith('.tar.tgz')) &&
  ...
  userGoals: z
    .string()
    .max(500, 'Goals/hints should be max 500 characters.')
    .optional(),
});
...
export function ProjectUploadForm() {
  ...
  const { isUploading: isUploadingFileWithHook, uploadFile } =
  ...
  const [errorLoadingClassrooms, setErrorLoadingClassrooms] = useState<
    string | null
  >(null);

  const [isConfirmAIDialogOpen, setIsConfirmAIDialogOpen] = useState(false);
  const [currentSubmissionForAI, setCurrentSubmissionForAI] =
    useState<ProjectSubmission | null>(null);
  const [currentUserGoalsForAI, setCurrentUserGoalsForAI] = useState<
    string | undefined
  >(undefined);
  ...
  const fetchAvailableClassrooms = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to fetch student's classrooms"
        );
      }
  ...
  const updateSubmissionStatusOnServer = useCallback(
    async (
      submissionId: string,
      ...
      try {
        ...
        const response = await fetch(
          `/api/projects/submissions/${submissionId}`,
          {
            method: 'PUT',
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message ||
              `Failed to update submission status to ${status}`
          );
        }
      } catch (error) {
        console.error(
          `Error updating submission status for ${submissionId}:`,
          error
        );
        throw error;
      }
    ...
    async (submission: ProjectSubmission, userGoals?: string) => {
      ...
      try {
        ...
        if (analysisOutput.error) {
          throw new Error(
            `Project structure analysis failed: ${analysisOutput.error} - ${analysisOutput.analysisSummary}`
          );
        }
        if (!analysisOutput.analyzedFileName) {
          // Should not happen if no error
          throw new Error(
            'Project structure analysis completed but returned no filename.'
          );
        }
        ...
        if (mapGenerationResult.error || !mapGenerationResult.conceptMapData) {
          throw new Error(
            `Concept map generation failed: ${mapGenerationResult.error || 'No map data produced.'}`
          );
        }

        // No need to parse mapGenerationResult.conceptMapData, it's already an object
        const parsedMapData: ConceptMapData =
          mapGenerationResult.conceptMapData;
        if (!parsedMapData.nodes || !parsedMapData.edges) {
          // Basic validation
          throw new Error(
            'AI output format error (missing nodes/edges after map generation step).'
          );
        }
        ...
        if (!mapCreateResponse.ok) {
          ...
          throw new Error(
            errorData.message || 'Failed to save AI-generated map.'
          );
        }
  ...
  const onSubmit = useCallback(
    async (values: z.infer<typeof projectUploadSchema>) => {
      if (!user) {
        toast({
      ...
      if (!values.projectFile || values.projectFile.length === 0) {
        toast({
          title: 'File Required',
          description: 'Please select a project archive file.',
          variant: 'destructive',
        });
        return;
      }
      ...
      const submissionPayload = {
        ...
        fileSize: file.size,
        classroomId:
          values.classroomId === NONE_CLASSROOM_VALUE
            ? null
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.message || 'Failed to create submission record'
          );
        }
        ...
        toast({
          ...
          description: `Record for "${file.name}" created. Confirm AI analysis next.`,
        });
        form.reset();
  ...
  const handleDeclineAIGeneration = useCallback(() => {
    toast({
      ...
      description: `Your project "${currentSubmissionForAI?.originalFileName}" was submitted but AI analysis was not initiated. You can track it in 'My Submissions'.`,
      duration: 7000,
    });
    setIsConfirmAIDialogOpen(false);
    setCurrentSubmissionForAI(null);
  ...
  const isBusyOverall =
    isUploadingFileWithHook || isSubmittingMetadata || isProcessingAIInDialog;
    ...
    <>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-8'>
            ...
            render={({ field: { onChange, ...fieldProps } }) => (
              <FormItem>
                ...
                <p className='text-sm text-muted-foreground'>
                  Accepted: {ACCEPTED_FILE_EXTENSIONS_STRING}. Max size:{' '}
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <Select
                  ...
                  value={field.value || ''}
                  disabled={isBusyOverall || isLoadingClassrooms}
                >
                  <FormControl>
                    <SelectTrigger>
                      <SelectValue
                  ...
                  <SelectContent>
                    ...
                    {!isLoadingClassrooms && !errorLoadingClassrooms && (
                      <>
                        <SelectItem value={NONE_CLASSROOM_VALUE}>
                          None (Personal Project)
                        </SelectItem>
                        {availableClassrooms.length === 0 && (
                          <SelectItem value='no_classrooms_available' disabled>
                            No classrooms enrolled in
                          </SelectItem>
                        )}
          ...
          />
          <Button
            id='tutorial-target-start-analysis-button'
            type='submit'
            className='w-full'
            disabled={
              isBusyOverall ||
              (!form.formState.isValid && !BYPASS_AUTH_FOR_TESTING) ||
              (!form.getValues('projectFile') && !BYPASS_AUTH_FOR_TESTING)
            }
          >
      ...
      {currentSubmissionForAI && (
        <AlertDialog
          open={isConfirmAIDialogOpen}
          onOpenChange={(open) => {
            if (!open && !isProcessingAIInDialog) {
              handleDeclineAIGeneration();
            }
            setIsConfirmAIDialogOpen(open);
          }}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle className='flex items-center'>
                <Brain className='mr-2 h-5 w-5 text-primary' /> AI Concept Map
                Generation
              </AlertDialogTitle>
              <AlertDialogDescription>
                ...
                <br />
                Would you like to proceed with AI-powered concept map generation
                now? This may take a few moments.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel
                onClick={handleDeclineAIGeneration}
