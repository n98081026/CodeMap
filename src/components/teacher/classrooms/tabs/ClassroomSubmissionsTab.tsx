...
import { Loader2, AlertTriangle, Eye, Inbox, ChevronLeft, ChevronRight } from "lucide-react"; // Removed FolderKanban, Added Chevrons
...
export const ClassroomSubmissionsTab: React.FC<ClassroomSubmissionsTabProps> = React.memo(function ClassroomSubmissionsTab({
  ...
  if (!isLoading && !error && submissions.length === 0) {
    return (
      <EmptyState
        icon={Inbox}
        title={totalSubmissionsCount === 0 ? "No Submissions Yet" : "No Submissions on This Page"}
        description={totalSubmissionsCount === 0 ? "Students in this classroom haven't submitted any projects for analysis." : "There are no submissions to display on the current page."}
      />
    );
  }
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex justify-center items-center space-x-4">
          ...
          <span className="text-sm text-muted-foreground">
            ...
            {totalSubmissionsCount !== undefined && ` (${totalSubmissionsCount} submissions)`}
          </span>
          <Button
            onClick={() => onPageChange(currentPage + 1)}
