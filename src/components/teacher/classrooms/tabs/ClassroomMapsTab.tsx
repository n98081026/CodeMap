...
import {
  ...
  ChevronLeft,
  ChevronRight,
} from 'lucide-react'; // Added ChevronLeft, ChevronRight, removed Library
...
export const ClassroomMapsTab: React.FC<ClassroomMapsTabProps> = React.memo(
  function ClassroomMapsTab({
    isLoading,
    error,
    maps,
    enrolledStudents,
    ...
    if (error && !isLoading) {
      // Show error only if not loading (prevents showing error during retry load)
      return (
        <EmptyState
          icon={AlertTriangle}
    ...
    if (!isLoading && !error && maps.length === 0) {
      return (
        <EmptyState
          icon={Share2Icon}
          title={
            totalMapsCount === 0 ? 'No Shared Maps Yet' : 'No Maps on This Page'
          }
          description={
            totalMapsCount === 0
              ? 'No concept maps have been shared with this classroom yet.'
              : 'There are no maps to display on the current page.'
          }
        />
      );
    }
              ...
              return (
                <TableRow key={map.id}>
                  ...
                  <TableCell className='text-right'>
                    ...
                    >
                      <Link
                        href={`/application/concept-maps/editor/${map.id}?viewOnly=true`}
                      >
