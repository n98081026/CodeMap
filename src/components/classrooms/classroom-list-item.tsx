...
export const ClassroomListItem: React.FC<ClassroomListItemProps> = React.memo(
  function ClassroomListItem({
    classroom,
    userRole,
    onEdit,
    onDelete,
    ...
    const linkTarget =
      detailLinkHref ||
      (isTeacherOrAdmin
        ? `/application/teacher/classrooms/${classroom.id}`
        : `/application/student/classrooms/${classroom.id}`);
      ...
      >
        <CardHeader>
          ...
          <CardDescription>
            ...
            {isTeacherOrAdmin && classroom.inviteCode && (
        ...
        <CardFooter
          className={cn(
            'grid gap-2',
            isTeacherOrAdmin ? 'grid-cols-3' : 'grid-cols-1'
          )}
        >
          <Button
            asChild
            variant={isTeacherOrAdmin ? 'default' : 'outline'}
            size='sm'
            className={cn(isTeacherOrAdmin && 'col-span-1')}
          >
            <Link href={linkTarget}>
              <ArrowRight className='mr-1 h-4 w-4 sm:mr-2' />{' '}
