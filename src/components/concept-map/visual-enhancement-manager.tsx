...
export function VisualEnhancementManager({
  ...
  const [currentLayoutType, setCurrentLayoutType] =
    useState<LayoutOptions['type']>('hierarchical');
      ...
      try {
        ...
        if (layoutEngineRef.current) {
          ...
          const layoutResult: LayoutResult =
            await layoutEngineRef.current.calculateLayout();
          ...
          <div className='grid grid-cols-2 md:grid-cols-5 gap-2'>
            {layoutOptions.map((layoutType) => (
              <Button
                key={layoutType}
                variant={
                  currentLayoutType === layoutType ? 'default' : 'outline'
                }
                size='sm'
