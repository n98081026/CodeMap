...
export const OptimizedAISuggestionPanel: React.FC<OptimizedAISuggestionPanelProps> = memo(({
  mapData,
  currentMapNodes = [],
  extractedConcepts = [],
  suggestedRelations = [],
  ...
  const filteredConcepts = useMemo(() => {
    return measure('filter-concepts', () => {
      ...
      return editableConcepts.filter(concept =>
  ...
  const relationsVirtualizer = useVirtualizedList({
    ...
    estimateSize: () => 60,
    overscan: 5,
    getItemKey: (index, item) => `${item.original.source}-${item.original.target}-${index}`,
  });
  ...
  const debouncedConceptEdit = useDebouncedCallback((index: number, value: string) => {
  ...
  const debouncedRelationEdit = useDebouncedCallback((index: number, value: string, field: 'source' | 'target' | 'relation') => {
  ...
  }), [filteredConcepts.length, filteredRelations.length, conceptsVirtualizer.metrics, relationsVirtualizer.metrics]);

  if (process.env.NODE_ENV === 'development') {
    console.log('AI Suggestion Panel Performance Metrics:', performanceMetrics);
  }
