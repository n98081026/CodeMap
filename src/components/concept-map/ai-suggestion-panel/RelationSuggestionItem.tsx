...
interface RelationSuggestionItemProps {
  ...
  onToggleEdit: (index: number, field: 'source' | 'target' | 'relation' | 'reason') => void;
  onInputChange: (index: number, value: string, field: 'source' | 'target' | 'relation' | 'reason') => void;
  onConfirmEdit: (index: number) => void;
  setDraggedRelationPreview: (relation: RelationSuggestion | null) => void;
}

const RelationSuggestionItem: React.FC<RelationSuggestionItemProps> = React.memo(({
  item,
  index,
  isViewOnlyMode,
  isSelected,
  ...
  return (
    <div
      className={cn(
        ...
        isSelected ? 'bg-purple-50 border-purple-200 dark:bg-purple-950 dark:border-purple-800' : 'bg-white border-gray-200 dark:bg-gray-800 dark:border-gray-700',
      ...
      onDragStart={(e) => {
        ...
        e.dataTransfer.setData('application/json', JSON.stringify(item.current));
    ...
    >
      ...
      <div className={cn(
        ...
        nodesExistOnMap ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'
      )}>
      ...
      <div className="flex-1 min-w-0 space-y-1">
        ...
        <div className="flex items-center gap-2 text-sm">
          <div className="flex-1 min-w-0">
            {renderEditableField('source', item.current.source, 'Source node')}
          </div>
          <ArrowRight className="h-3 w-3 text-muted-foreground flex-shrink-0" />
          <div className="flex-1 min-w-0">
            {renderEditableField('target', item.current.target, 'Target node')}
          </div>
        </div>

        {/* Relation label */}
        <div className="text-sm font-medium">
          {renderEditableField('relation', item.current.relation, 'Relation label')}
        </div>

        {/* Reason */}
        {item.current.reason && (
          <div className="text-xs text-muted-foreground">
            <span className="font-medium">Reason:</span> {item.current.reason}
          </div>
        )}
      </div>
    </div>
