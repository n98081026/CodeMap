...
const SuggestedIntermediateNode: React.FC<NodeProps<SuggestedIntermediateNodeData>> = ({
  ...
  const { addNode, addEdge, deleteEdge, removeStructuralSuggestion, findEdgeByNodes } = useConceptMapStore.getState();
  ...
  const onAccept = () => {
    ...
    if (suggestionData.originalEdgeId) {
      ...
      const edgeToDelete = findEdgeByNodes(suggestionData.sourceNodeId, suggestionData.targetNodeId);
      if (edgeToDelete) {
        deleteEdge(edgeToDelete.id);
      } else {
        toast({
          title: "Warning",
          description: "Original edge for intermediate node suggestion not found or already removed.",
    ...
    toast({ title: "Suggestion applied", description: "Intermediate node added." });
    ...
    <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <Handle type="target" position={Position.Top} className="w-1.5 h-1.5 !bg-blue-400/70" />
          <Handle type="source" position={Position.Bottom} className="w-1.5 h-1.5 !bg-blue-400/70" />
          <Handle type="target" position={Position.Left} className="w-1.5 h-1.5 !bg-blue-400/70" />
          <Handle type="source" position={Position.Right} className="w-1.5 h-1.5 !bg-blue-400/70" />
        </Card>
      </PopoverTrigger>
        ...
        <div className="space-y-2">
          ...
          {suggestionData.labelToIntermediate && (
            <p className="text-xs text-muted-foreground">
              Edge 1: Source Node → "{suggestionData.labelToIntermediate}" → New Node
            </p>
          )}
          {suggestionData.labelFromIntermediate && (
            <p className="text-xs text-muted-foreground">
              Edge 2: New Node → "{suggestionData.labelFromIntermediate}" → Target Node
            </p>
          )}
          {reason && (
            <>
