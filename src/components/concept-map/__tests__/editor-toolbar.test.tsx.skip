...
describe('EditorToolbar (/components/concept-map/editor-toolbar.tsx)', () => {
  ...
  describe('when in view-only mode', () => {
    ...
    it('should disable the save button', () => {
      expect(screen.getByTitle(/Save Map \(Disabled in View Mode\)/i)).toBeDisabled();
    });

    it('should disable the add node button', () => {
      expect(screen.getByTitle(/Add Node \(Disabled\)/i)).toBeDisabled();
    });

    it('should disable the undo button', () => {
      expect(screen.getByTitle(/Undo \(Disabled\)/i)).toBeDisabled();
    });

    it('should disable the redo button', () => {
      expect(screen.getByTitle(/Redo \(Disabled\)/i)).toBeDisabled();
    });

    it('should not disable the new map button', () => {
      expect(screen.getByTitle('New map')).not.toBeDisabled();
    });

    it('should not disable the export map button', () => {
      expect(screen.getByTitle('Export Map (JSON)')).not.toBeDisabled();
    });

    it('should not disable the show properties button', () => {
      expect(screen.getByTitle(/Show Properties/i)).not.toBeDisabled();
    });

    it('should not disable the show AI suggestions button', () => {
  ...
  it('Undo and Redo buttons should be disabled based on canUndo/canRedo props', async () => {
    ...
    // When enabled, the title should not have the "(Disabled)" suffix.
