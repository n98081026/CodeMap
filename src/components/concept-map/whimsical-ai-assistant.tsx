...
export function WhimsicalAIAssistant({
  ...
  const { mapData, selectedElementId, multiSelectedNodeIds } = useConceptMapStore(
    useCallback(s => ({
      mapData: s.mapData,
  ...
  const handleNaturalInput = useCallback(async () => {
    ...
    try {
      ...
      // 簡單的意圖識別
      if (input.includes('添加') || input.includes('概念') || input.includes('提取')) {
        onExtractConcepts?.(naturalInput);
        toast({ 
          title: "AI 理解了你的需求", 
          description: "正在提取相關概念..." 
        });
      } else if (input.includes('關係') || input.includes('連接') || input.includes('關聯')) {
        onSuggestRelations?.();
        toast({ 
          title: "AI 理解了你的需求", 
          description: "正在分析概念關係..." 
        });
      } else if (input.includes('整理') || input.includes('佈局') || input.includes('排列')) {
        onSmartLayout?.();
        toast({ 
          title: "AI 理解了你的需求", 
          description: "正在優化地圖佈局..." 
        });
      } else if (input.includes('摘要') || input.includes('總結') || input.includes('概述')) {
        onGenerateSummary?.();
        toast({ 
          title: "AI 理解了你的需求", 
  ...
  }, [naturalInput, isViewOnlyMode, onExtractConcepts, onSuggestRelations, onSmartLayout, onGenerateSummary, toast]);
