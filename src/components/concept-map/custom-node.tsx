...
const CustomNodeComponent: React.FC<NodeProps<CustomNodeData>> = ({
  data,
  id,
  selected,
  xPos,
  ...
  const [toolbarPosition, setToolbarPosition] = useState<'above' | 'below'>(
    'above'
  );
  ...
  const handleEditLabelForToolbar = useCallback(
    () => setEditingNodeId(id),
    [setEditingNodeId, id]
  );
  const handleChangeColorForToolbar = useCallback(
  ...
  const handleDeleteNodeForToolbar = useCallback(
    () => deleteNode(id),
    [deleteNode, id]
  );
  ...
  const nodeStyle: React.CSSProperties = {
    ...
    backgroundColor:
      data.type === 'ai-group-parent'
        ? 'rgba(100, 116, 139, 0.05)'
        : data.backgroundColor || 'hsl(var(--card-values))',
  };

  const handleNodeDoubleClick = () => {
    if (!nodeIsViewOnly && !data.isGhost && !data.isStaged && data.label) {
      // Prevent editing staged/ghost nodes via double click
      setEditingNodeId(id);
    }
  };
  ...
  useEffect(() => {
    if (selected && nodeRef.current) {
      ...
      if (toolbarCalculatedLeft < VIEWPORT_MARGIN) {
        ...
        newHorizontalOffset =
          viewportWidth - VIEWPORT_MARGIN - toolbarCalculatedRight;
      }
  ...
  return (
    <div
      ...
      className={cn(
        ...
        selected && !isBeingProcessedByAI && !data.isStaged && !data.isGhost
          ? 'ring-2 ring-primary'
          : '',
        ...
        data.isGhost &&
          'border-dotted border-purple-500 opacity-60 bg-purple-500/10 is-ghost-node',
        data.isDimmed && 'opacity-50 transition-opacity duration-300',
        data.type === 'ai-group-parent' &&
          'border-2 border-dashed border-slate-500/30 dark:border-slate-600/50'
      )}
    ...
    >
      ...
      {!nodeIsViewOnly &&
        !data.isGhost &&
        !data.isStaged && ( // Ensure move handle doesn't appear on ghost/staged
          <MoveIcon
      ...
      <Card
        ...
        className={cn(
          ...
          data.shape === 'ellipse'
            ...
            : 'rounded-lg',
          typeof nodeWidth === 'number' || data.width
            ? ''
            : `min-w-[${NODE_MIN_WIDTH}px] max-w-[${NODE_MAX_WIDTH}px]`,
          typeof nodeHeight === 'number' || data.height
            ? ''
            : `min-h-[${NODE_MIN_HEIGHT}px]`,
      ...
      >
        ...
        !nodeIsViewOnly &&
        !data.isGhost &&
        !data.isStaged ? ( // Check isGhost and isStaged here too
          ...
          <>
            <CardHeader
              className={cn(
                ...
                data.shape === 'ellipse' &&
                  'justify-center items-center flex-col text-center'
              )}
            >
            ...
            {data.details && data.shape !== 'ellipse' && (
              <CardContent
                className={cn(
                  ...
                  typeof nodeHeight === 'number' || data.height
                    ? 'overflow-y-auto'
                    : `max-h-[${NODE_DETAILS_MAX_HEIGHT}px] overflow-y-auto`
                )}
              >
      ...
      <Handle
        ...
        id={`${id}-top-source`}
        className='react-flow__handle-custom !-top-1.5 w-3 h-3 bg-background border border-primary rounded-full'
        isConnectable={!nodeIsViewOnly && !data.isGhost && !data.isStaged}
      />
      <Handle
        type='target'
        position={Position.Top}
      ...
      <Handle
        ...
        id={`${id}-right-source`}
        className='react-flow__handle-custom !-right-1.5 w-3 h-3 bg-background border border-primary rounded-full'
        isConnectable={!nodeIsViewOnly && !data.isGhost && !data.isStaged}
      />
      <Handle
        type='target'
      ...
      <Handle
        ...
        id={`${id}-bottom-source`}
        className='react-flow__handle-custom !-bottom-1.5 w-3 h-3 bg-background border border-primary rounded-full'
        isConnectable={!nodeIsViewOnly && !data.isGhost && !data.isStaged}
      />
      <Handle
        type='target'
      ...
      <Handle
        ...
        id={`${id}-left-source`}
        className='react-flow__handle-custom !-left-1.5 w-3 h-3 bg-background border border-primary rounded-full'
        isConnectable={!nodeIsViewOnly && !data.isGhost && !data.isStaged}
      />
      <Handle
        type='target'
      ...
      {!nodeIsViewOnly &&
        ...
        !data.isGhost &&
        !data.isStaged &&
        data.onAddChildNodeRequest &&
        hoverButtonPositions.map((btn) => (
          <button
            key={btn.pos}
            onClick={(e) => {
              e.stopPropagation();
