...
const getManhattanPath = (
  ...
  if (sourcePosition === Position.Left)
    sourceAnchor = { x: currentX - STRAIGHT_EXIT_LENGTH, y: currentY };
  else if (sourcePosition === Position.Right)
    sourceAnchor = { x: currentX + STRAIGHT_EXIT_LENGTH, y: currentY };
  else if (sourcePosition === Position.Top)
    sourceAnchor = { x: currentX, y: currentY - STRAIGHT_EXIT_LENGTH };
  ...
  if (targetPosition === Position.Left)
    targetAnchor = { x: targetX - STRAIGHT_EXIT_LENGTH, y: targetY };
  else if (targetPosition === Position.Right)
    targetAnchor = { x: targetX + STRAIGHT_EXIT_LENGTH, y: targetY };
  else if (targetPosition === Position.Top)
    targetAnchor = { x: targetX, y: targetY - STRAIGHT_EXIT_LENGTH };
  ...
  for (let i = 1; i < pathPoints.length; i++) {
    ...
    if (i < pathPoints.length - 1) {
      ...
      if (
        seg1Length >= CORNER_RADIUS &&
        seg2Length >= CORNER_RADIUS &&
        CORNER_RADIUS > 0
      ) {
        const dx1 = p2.x - p1.x;
        const dy1 = p2.y - p1.y;
  ...
  if (intermediateSegments.length >= 2) {
    ...
    // Fallback to the middle segment if only one (e.g. L-shape)
...
export const getMarkerDefinition = (
  markerTypeString?: string,
  edgeColor?: string
  ...
  switch (markerTypeString.toLowerCase()) {
    case 'arrow':
      return { type: MarkerType.Arrow, color, strokeWidth: 1 };
    case 'arrowclosed':
      return { type: MarkerType.ArrowClosed, color, strokeWidth: 1 };
    default:
      return undefined;
  }
};
...
export const OrthogonalEdge: React.FC<EdgeProps<OrthogonalEdgeData>> =
  React.memo(function OrthogonalEdge({
    id,
    sourceX,
    sourceY,
    targetX,
      ...
      <>
        ...
        <EdgeLabelRenderer>
          <div
            ...
            className={cn(
              'nodrag nopan p-1 rounded-md group',
              isEditingLabel
                ? 'bg-background shadow-lg border'
                : 'hover:bg-muted/70',
              selected && !isEditingLabel && 'bg-muted/90'
            )}
            ...
            {!isEditingLabel && currentLabelValue && !isViewOnlyMode && (
              <div className='absolute -top-2 -right-2 opacity-0 group-hover:opacity-100 transition-opacity'>
                <Button
                  variant='ghost'
                  size='icon'
                  className='h-5 w-5 p-0.5 bg-background/80 hover:bg-muted'
