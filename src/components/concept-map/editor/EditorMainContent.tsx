...
interface EditorMainContentProps {
  ...
  // Event handlers
  handleFlowSelectionChange: (id: string | null, type: 'node' | 'edge' | null) => void;
  ...
  handlePaneContextMenuRequest: (event: React.MouseEvent, position: { x: number; y: number }) => void;
  handleConceptSuggestionDrop: (conceptItem: any, position: { x: number; y: number }) => void;
      ...
      onEdgesDeleteInStore={(edgeIds) => {
        if (Array.isArray(edgeIds)) {
          edgeIds.forEach((edgeId) =>
            ...
            require('@/stores/concept-map-store').useConceptMapStore.getState().deleteEdge(edgeId)
          );
        }
      }}
      onConnectInStore={(params) =>
        // Note: This should be passed as a prop instead of direct store access
        require('@/stores/concept-map-store').useConceptMapStore.getState().addEdge(params)
      }
