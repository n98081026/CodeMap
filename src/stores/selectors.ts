...
// Undo/Redo selectors
export const selectCanUndo = (state: ConceptMapState) => {
  // Check if temporal state exists and has past states
  return (state as any).temporal?.pastStates?.length > 0 || false;
};

export const selectCanRedo = (state: ConceptMapState) => {
  // Check if temporal state exists and has future states
  return (state as any).temporal?.futureStates?.length > 0 || false;
};
