...
const storeDefinition: StateCreator<ConceptMapState> = (set, get) => ({
  ...
  deleteEdge: (edgeId) =>
    set((state) => ({
      ...
      selectedElementType:
        state.selectedElementId === edgeId ? null : state.selectedElementType,
    })),
    ...
    set((state) => {
      ...
      if (
        ...
        stagedData.nodes.forEach((node, index) => {
          const permanentId =
            finalNodes[finalNodes.length - (stagedData.nodes.length - index)]
  ...
  updateDragPreviewPosition: (position: { x: number; y: number } | null) => {
  ...
  applyFormGroupSuggestion: (nodeIds, groupName, overlayGeometry) => {
    ...
    if (
      ...
      if (groupNodes.length > 0) {
        ...
        groupNodeY =
          groupNodes.reduce((acc, n) => acc + n.y!, 0) / groupNodes.length - 50; // Offset upwards
      }
    }
    ...
    nodeIds.forEach((nodeId) => {
      const nodeToUpdate = get().mapData.nodes.find(
        (n: ConceptMapNode) => n.id === nodeId
      );
      if (nodeToUpdate) {
