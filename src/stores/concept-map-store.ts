...
export interface StagedMapDataWithContext {
  ...
  actionType?: 'intermediateNode' | 'summarizeNodes' | 'quickCluster' | 'generateSnippet' | 'aiTidyUpComplete';
    ...
    (set, get) => ({
      ...
      finishConnectionAttempt: (targetNodeId) => {
        ...
        if (sourceNodeId && targetNodeId) {
            get().addEdge({ source: sourceNodeId, target: targetNodeId, label: "connects" });
        }
      ...
      importMapData: (importedData, fileName) => {
        const newName = fileName ? fileName.replace(/\.json$/i, '') : `Imported Map`;
        get().addDebugLog(`[STORE importMapData] New Name: ${newName}`);
        set((state) => ({
            ...initialStateBase,
            mapData: importedData,
            mapName: newName,
            mapId: state.isNewMapMode ? 'new' : (state.mapId || `imported-${Date.now()}`),
            isViewOnlyMode: false,
            isLoading: false,
            initialLoadComplete: true,
            error: null,
      ...
      addNode: (options) => {
        ...
        const newNode: ConceptMapNode = {
            id: newNodeId,
            text: options.text, type: options.type,
            x: options.position.x, y: options.position.y,
            details: options.details || '',
            parentNode: options.parentNode, childIds: [],
            backgroundColor: options.backgroundColor,
            shape: options.shape || 'rectangle',
            width: options.width ?? 150, height: options.height ?? 70
        };
        set((state) => {
      ...
      deleteNode: (nodeIdToDelete) => {
        ...
        set((state) => {
          ...
          const graphInstance = graphUtil.fromArrays(state.mapData.nodes, state.mapData.edges);
          if (!graphInstance.hasNode(nodeIdToDelete)) { return state; }
          ...
          const finalNodesToKeep = state.mapData.nodes.filter(node => !nodesToDeleteSet.has(node.id)).map(node => ({ ...node, childIds: node.childIds?.filter(childId => !nodesToDeleteSet.has(childId)) }));
      ...
      addEdge: (options) => {
        ...
        const newEdge: ConceptMapEdge = {
            ...
            sourceHandle: options.sourceHandle || null, targetHandle: options.targetHandle || null,
            label: options.label || 'connects',
            color: options.color, lineType: options.lineType || 'solid',
      ...
      deleteEdge: (edgeId) => set((state) => ({
          ...
          selectedElementId: state.selectedElementId === edgeId ? null : state.selectedElementId,
          selectedElementType: state.selectedElementId === edgeId ? null : state.selectedElementType
      })),
      setStagedMapData: (data: StagedMapDataWithContext | null) => set({ stagedMapData: data, isStagingActive: !!data, ghostPreviewData: null }),
      ...
      completeConnectionMode: (targetNodeId?: string, targetHandleId?: string | null) => {
      ...
      updateDragPreviewPosition: (position) => set({ dragPreviewPosition: position as {x:number, y:number} | null }),
      ...
      fetchProjectOverview: async (input) => {
        ...
        try {
          ...
          set({ projectOverviewData: { overallSummary: "Failed to generate overview.", keyModules: [], error: e.message }, isFetchingOverview: false, error: `Overview Error: ${e.message}` });
        }
      },
      ...
      setGhostPreview: (nodesToPreview) => {
        ...
        const previewNodesWithOriginalData = nodesToPreview.map(previewNode => {
          ...
          return { ...previewNode, originalX: originalNode?.x, originalY: originalNode?.y, width: originalNode?.width || previewNode.width || 150, height: originalNode?.height || previewNode.height || 70 };
        });
        set({ ghostPreviewData: { nodes: previewNodesWithOriginalData }, stagedMapData: null, isStagingActive: false });
      ...
      acceptGhostPreview: () => {
        ...
        const updates: LayoutNodeUpdate[] = previewData.nodes.map(node => ({ id: node.id, x: node.x, y: node.y }));
      ...
      findEdgeByNodes: (sourceId, targetId) => {
        return get().mapData.edges.find(edge => (edge.source === sourceId && edge.target === targetId) || (edge.source === targetId && edge.target === sourceId));
      },
