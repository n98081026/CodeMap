...
2.  **Create a Brand New Workspace (Crucial Step):**
    ...
    - Clone the repository into a **new folder** with a fresh name:
      ...
      _(Replace `<repository_url>` with the project's actual Git repository URL, and `<new_project_directory_name>` with a new name like `project-fresh-start` or `concept-map-clean-env`)_.
...
6.  **Verify Access:** Perform a simple read operation (e.g., `ls` in the new project root) to confirm tools are targeting the new directory.
7.  **Systematic Code Re-application:**
...
8.  **Final Verification:** Attempt the previously failing UI task as a key test:
...
9.  **Submit Restored Work:** If all re-application and verification steps are successful, commit the consolidated, restored work to a new branch in the clean repository.
10. **Await Further Instructions:** Report completion to you and await direction for new development tasks based on the updated `TODO.md`.
